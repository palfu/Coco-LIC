...
inline int opencv_buildOpticalFlowPyramid(cv::InputArray img,
...
void calculate_optical_flow(cv::InputArray prevImg,
...
inline int calculate_LK_optical_flow(const cv::Range &range,
...
struct opencv_LKTrackerInvoker : cv::ParallelLoopBody {
    opencv_LKTrackerInvoker(const cv::Mat *_prevImg,
                            const cv::Mat *_prevDeriv,
                            const cv::Mat *_nextImg,
                            const cv::Point2f *_prevPts,
                            cv::Point2f *_nextPts,
...
public:
    ...
    **/
    void calc(cv::InputArray prevImg,
              cv::InputArray nextImg,
              cv::InputArray prevPts,
              cv::InputOutputArray nextPts,
              cv::OutputArray status,
    ...
    void allocate_img_deriv_memory(std::vector<cv::Mat> &img_pyr, std::vector<cv::Mat> &img_pyr_deriv_I, std::vector<cv::Mat> &img_pyr_deriv_I_buff);
    void calc_image_deriv_Sharr(std::vector<cv::Mat> &img_pyr, std::vector<cv::Mat> &img_pyr_deriv_I, std::vector<cv::Mat> &img_pyr_deriv_I_buff);
...
public:
    ...
    int track_image(const cv::Mat &curr_img,
