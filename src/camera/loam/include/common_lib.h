...
inline Eigen::Matrix<T, 3, 3> right_jacobian_of_rotion_matrix(const Eigen::Matrix<T, 3, 1> &omega)
{
    ...
    T theta = omega.norm();
    if (std::isnan(theta) || theta == 0) return Eigen::Matrix<T, 3, 3>::Identity();
...
Eigen::Matrix<T, 3, 3> inverse_right_jacobian_of_rotion_matrix(const Eigen::Matrix<T, 3, 1> &omega)
{
    ...
    Eigen::Matrix<T, 3, 3> hat_a = vec_to_hat(a);
    res_mat_33 =
        (theta / 2) * (cot(theta / 2)) * Eigen::Matrix<T, 3, 3>::Identity() + (1 - (theta / 2) * (cot(theta / 2))) * a * a.transpose() + (theta / 2) * hat_a;
    // cout << "Omega: " << omega.transpose() << endl;
    // cout << "Res_mat_33:\r\n"  <<res_mat_33 << endl;
    return res_mat_33;
}
...
struct Camera_Lidar_queue {
    ...
    std::deque<PointCloudMsgPtr> *m_liar_frame_buf = nullptr;

    double time_wrt_first_imu_time(double &time) { return time - m_first_imu_time; }
    ...
    double get_lidar_front_time()
    {
        if (m_liar_frame_buf != nullptr && m_liar_frame_buf->size()) {
            m_last_lidar_time = m_liar_frame_buf->front()->timestamp_ + 0.1;
    ...
    double get_camera_front_time() { return m_last_visual_time + m_camera_imu_td; }
