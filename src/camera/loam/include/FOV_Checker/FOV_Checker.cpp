...
void FOV_Checker::check_fov(Eigen::Vector3d cur_pose, Eigen::Vector3d axis, double theta, double depth, vector<BoxPointType> &boxes)
{
    ...
    for (i = start_i; i <= maxn; i++) {
        ...
        if (index == 1 || index == 4) {
            start_point = Eigen::Vector3d(center_point(0), floor(center_point(1) / box_length + eps_value) * box_length,
                                          floor(center_point(2) / box_length + eps_value) * box_length);
            ...
            if (index == 2 || index == 5) {
                start_point = Eigen::Vector3d(floor(center_point(0) / box_length + eps_value) * box_length, center_point(1),
                                              floor(center_point(2) / box_length + eps_value) * box_length);
                ...
                start_point = Eigen::Vector3d(floor(center_point(0) / box_length + eps_value) * box_length,
                                              floor(center_point(1) / box_length + eps_value) * box_length, center_point(2));
                max_uN      = ceil((env.vertex_max[1] - env.vertex_min[1]) / box_length);
                max_ulogN   = floor(log2(max_uN));
                max_vN      = ceil((env.vertex_max[2] - env.vertex_min[2]) / box_length);
                plane_u_min = env.vertex_min[1];
...
bool FOV_Checker::check_box(Eigen::Vector3d cur_pose, Eigen::Vector3d axis, double theta, double depth, const BoxPointType box)
{
    ...
    if (check_surface(cur_pose, axis, theta, depth, plane[0]) || check_surface(cur_pose, axis, theta, depth, plane[1]) ||
        check_surface(cur_pose, axis, theta, depth, plane[2]) || check_surface(cur_pose, axis, theta, depth, plane[3]) ||
        check_surface(cur_pose, axis, theta, depth, plane[4]) || check_surface(cur_pose, axis, theta, depth, plane[5]))
        s = 1;
    else
        s = 0;
    return s;
...
bool FOV_Checker::check_surface(Eigen::Vector3d cur_pose, Eigen::Vector3d axis, double theta, double depth, PlaneType plane)
{
    ...
    if (check_line(cur_pose, axis, theta, depth, plane_p, plane_u) || check_line(cur_pose, axis, theta, depth, plane_p, plane_v) ||
        check_line(cur_pose, axis, theta, depth, plane_p + plane_u, plane_v) || check_line(cur_pose, axis, theta, depth, plane_p + plane_v, plane_u)) {
        s = 1;
        return s;
    }
...
{
    if (box.vertex_min[0] >= env.vertex_min[0] - eps_value && box.vertex_min[1] >= env.vertex_min[1] - eps_value &&
        box.vertex_min[2] >= env.vertex_min[2] - eps_value && box.vertex_max[0] <= env.vertex_max[0] + eps_value &&
        box.vertex_max[1] <= env.vertex_max[1] + eps_value && box.vertex_max[2] <= env.vertex_max[2] + eps_value) {
        return true;
    } else {
        return false;
    }
