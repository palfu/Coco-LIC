...
void ImuProcess::IMU_Initial(const MeasureGroup &meas, StatesGroup_r3live &state_inout, int &N)
{
    ...
    for (const auto &imu : meas.imu) {
        const auto &cur_acc = imu->acc_;
        const auto &cur_gyr = imu->gyro_;
...
void ImuProcess::lic_state_propagate(const MeasureGroup &meas, StatesGroup_r3live &state_inout)
{
...
// Avoid abnormal state input
void check_in_out_state(const StatesGroup_r3live &state_in, StatesGroup_r3live &state_inout)
{
...
StatesGroup_r3live ImuProcess::imu_preintegration(const StatesGroup_r3live &state_in, std::deque<ImuMsgPtr> &v_imu, double end_pose_dt)
{
    std::unique_lock<std::mutex> lock(g_imu_premutex);
    StatesGroup_r3live state_inout = state_in;
    ...
    for (auto it_imu = v_imu.begin(); it_imu != (v_imu.end() - 1); it_imu++) {
        ...
        angvel_avr << 0.5 * (head->gyro_.x() + tail->gyro_.x()), 0.5 * (head->gyro_.y() + tail->gyro_.y()), 0.5 * (head->gyro_.z() + tail->gyro_.z());
        acc_avr << 0.5 * (head->acc_.x() + tail->acc_.x()), 0.5 * (head->acc_.y() + tail->acc_.y()), 0.5 * (head->acc_.z() + tail->acc_.z());

        angvel_avr -= state_inout.bias_g;
        acc_avr = acc_avr - state_inout.bias_a;

        if (tail->timestamp_ < state_inout.last_update_time) {
            continue;
        }

        if (if_first_imu) {
            if_first_imu = 0;
            dt           = tail->timestamp_ - state_inout.last_update_time;
        } else {
        ...
        Eigen::Matrix3d Exp_f = Exp_r3live(angvel_avr, dt);
    ...
    // cout <<__FILE__ << ", " << __LINE__ <<" ,diagnose lio_state = " << std::setprecision(2) <<(state_inout - StatesGroup_r3live()).transpose() << endl;
    /*** calculated the pos and attitude prediction at the frame-end ***/
    dt = end_pose_dt;

    state_inout.last_update_time = v_imu.back()->timestamp_ + dt;
    ...
    state_inout.rot_end = R_imu * Exp_r3live(angvel_avr, dt);
...
void ImuProcess::lic_point_cloud_undistort(const MeasureGroup &meas, const StatesGroup_r3live &_state_inout, PointCloudXYZINormal &pcl_out)
{
    StatesGroup_r3live state_inout = _state_inout;
    auto v_imu                     = meas.imu;
    v_imu.push_front(last_imu_);
    const double &imu_end_time = v_imu.back()->timestamp_;
    const double &pcl_beg_time = meas.lidar_beg_time;
    ...
    */
    ...
    for (auto it_imu = v_imu.begin(); it_imu != (v_imu.end() - 1); it_imu++) {
        ...
        acc_avr     = 0.5 * (head->acc_ + tail->acc_);
        angvel_avr -= state_inout.bias_g;
        acc_avr = acc_avr - state_inout.bias_a;
...
// fout<<head->header.stamp.toSec()<<" "<<angvel_avr.transpose()<<" "<<acc_avr.transpose()<<std::endl;
    ...
    for (auto it_kp = IMU_pose.end() - 1; it_kp != IMU_pose.begin(); it_kp--) {
        ...
        for (; it_pcl->curvature / double(1000) > head->offset_time; it_pcl--) {
            ...
            Eigen::Matrix3d R_i(R_imu * Exp_r3live(angvel_avr, dt));
...
void ImuProcess::Process(const MeasureGroup &meas, StatesGroup_r3live &stat, PointCloudXYZINormal::Ptr cur_pcl_un_)
{
    // double t1, t2, t3;
    // t1 = omp_get_wtime();
    ...
    if (imu_need_init_) {
        ...
        if (init_iter_num > MAX_INI_COUNT) {
            ...
            printf("IMU Initials: Gravity: %.4f %.4f %.4f; state.bias_g: %.4f %.4f %.4f; acc covarience: %.8f %.8f %.8f; gry covarience: %.8f %.8f %.8f\n",
                   stat.gravity[0], stat.gravity[1], stat.gravity[2], stat.bias_g[0], stat.bias_g[1], stat.bias_g[2], cov_acc[0], cov_acc[1], cov_acc[2],
