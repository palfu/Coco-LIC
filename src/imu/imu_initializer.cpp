...
IMUInitializer::IMUInitializer(const YAML::Node &node) : initial_done_(false)
{
    ...
    imu_excite_threshold_ = yaml::GetValue<double>(node, "imu_excite_threshold", 1.0);
...
bool IMUInitializer::StaticInitialIMUState()
{
    ...
    if (imu_datas_.back().timestamp - imu_datas_.begin()->timestamp < window_length_) return false;
    ...
    for (size_t i = imu_datas_.size() - 1; i >= 0; i--) {
        ...
        if (imu_datas_.back().timestamp - imu_datas_[i].timestamp >= window_length_) break;
    }
