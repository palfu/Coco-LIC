...
namespace cocolic {
ImuStateEstimator::ImuStateEstimator(const YAML::Node& node) : motion_state_(MotionState::start_motionless)
{
    accel_excite_threshold_ = yaml::GetValue<double>(node, "accel_excite_threshold", 0.5);
    gyro_excite_threshold_  = yaml::GetValue<double>(node, "gyro_excite_threshold", 0.5);
}
...
void ImuStateEstimator::Propagate(const IMUState& imu_state, double from_timestamp, double to_timestamp)
{
    if (imu_data_.empty()) {
        LOG(WARNING) << " no imu data !";
    }
    if (imu_data_.front().timestamp > to_timestamp) {
        LOG(WARNING) << " input time too small." << to_timestamp << "|" << imu_data_.front().timestamp;
    }
    ...
    if (motion_state_ != MotionState::start_motionless) {
        ...
        for (size_t i = 1; i < imu_cache.size(); i++) {
            ...
            Eigen::Vector3d un_acc_0 = x_now.q * (last_imu.accel - bias.accel_bias) - x_now.g;
            Eigen::Vector3d un_gyro  = 0.5 * (last_imu.gyro + imu_cache[i].gyro) - bias.gyro_bias;
            x_now.q *= Eigen::Quaterniond(1, un_gyro(0) * dt / 2, un_gyro(1) * dt / 2, un_gyro(2) * dt / 2).normalized();
            Eigen::Vector3d un_acc_1 = x_now.q * (imu_cache[i].accel - bias.accel_bias) - x_now.g;
    ...
    LOG(INFO) << "[IMU Propagate] desire time [" << from_timestamp << ", " << to_timestamp << "]; actual time [" << imu_cache.front().timestamp << ", "
              << imu_cache.back().timestamp << "]";
    LOG(INFO) << "[IMU Propagate] start pose: " << VectorToString(propagate_start_state_.p) << "; end pose: " << VectorToString(latest_state_.p);

    UpdateMotionState(imu_state, imu_cache);
}

//
void ImuStateEstimator::UpdateMotionState(const IMUState& imu_state, const Eigen::aligned_vector<IMUData>& imu_cache)
{
    ...
    for (size_t i = 0; i < imu_cache.size(); i++) {
        accel_var += (imu_cache[i].accel - accel_avg).dot(imu_cache[i].accel - accel_avg);
        gyro_var += (imu_cache[i].gyro - gyro_avg).dot(imu_cache[i].gyro - gyro_avg);
    }
    accel_var = std::sqrt(accel_var / ((double) imu_cache.size() - 1));
    gyro_var  = std::sqrt(gyro_var / ((double) imu_cache.size() - 1));

    if (motion_state_ == MotionState::start_motionless) {
        if (accel_var >= accel_excite_threshold_) motion_state_ = MotionState::moving;
    } else {
        //
