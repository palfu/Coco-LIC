...
void Trajectory::UndistortScan(const PosCloud &scan_raw, const int64_t target_timestamp, PosCloud &scan_in_target) const
{
...
void Trajectory::UndistortScanInG(const PosCloud &scan_raw, const int64_t scan_raw_timestamp, PosCloud &scan_in_target) const
{
    scan_in_target.header = scan_raw.header;
    scan_in_target.resize(scan_raw.size());
    scan_in_target.is_dense = true;
...
SE3d Trajectory::GetSensorPose(const double timestamp, const ExtrinsicParam &EP_StoI) const
{
    double time_ns = timestamp * S_TO_NS + EP_StoI.t_offset_ns;

    if (!(time_ns >= this->minTimeNs() && time_ns < this->maxTimeNs())) {
        std::cout << time_ns << "; not in [" << this->minTimeNs() << ", " << this->maxTimeNs() << "]; "
                  << "input time: " << timestamp << "[s]; t_offset: " << EP_StoI.t_offset_ns << " [ns]\n";
    }
    assert(time_ns >= this->minTimeNs() && time_ns < this->maxTimeNs() && "[GetSensorPose] querry time not in range.");
...
SE3d Trajectory::GetSensorPoseNURBS(const int64_t timestamp, const ExtrinsicParam &EP_StoI) const
{
    int64_t time_ns = timestamp;

    assert(time_ns >= 0 && time_ns < this->maxTimeNsNURBS() && "[GetSensorPose] querry time not in range.");
...
SE3d Trajectory::GetSensorPoseNURBS(const int64_t timestamp, const ExtrinsicParam &EP_StoI, int start_idx) const
{
    int64_t time_ns = timestamp;
...
void Trajectory::ToTUMTxt(std::string traj_path, int64_t maxtime, bool is_evo_viral, double dt)
{
    ...
    for (int64_t t = min_time; t < max_time; t += dt_ns) {
        ...
        outfile.precision(5);
        outfile << p(0) << " " << p(1) << " " << p(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << "\n";
    }
    outfile.close();
    std::cout << "\n🍺 Save trajectory at " << traj_path << std::endl;

    Eigen::AngleAxisd rotation_vector(start_end.unit_quaternion());
    std::cout << "   Start-to-end deviation: " << std::setprecision(3) << start_end.translation().norm() << "m, " << rotation_vector.angle() * 180 / M_PI
              << "°." << std::endl;
}
