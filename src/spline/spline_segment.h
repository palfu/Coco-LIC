...
struct SplineSegmentMeta : public MetaData {
    ...
    SplineSegmentMeta(int64_t _t0_ns, int64_t _dt_ns, size_t _n = 0) : t0_ns(_t0_ns), dt_ns(_dt_ns), n(_n)
    {
        pow_inv_dt[0] = 1.0;
    ...
    std::pair<double, size_t> computeTIndexNs(int64_t time_ns) const
    {
        ...
        assert(time_ns >= MinTimeNs() && time_ns < MaxTimeNs() && "computeTIndexNs from SplineSegmentMeta");
...
struct SplineMeta {
    ...
    bool ComputeSplineIndex(const int64_t& time_ns, size_t& idx, double& u) const
    {
        idx = 0;
        ...
        std::cout << std::fixed << std::setprecision(15) << "ComputeSplineIndex1 Problem :" << time_ns << " not in [" << segments[0].t0_ns << ", "
                  << segments[0].MaxTimeNs() << "]\n";
        return false;
    }
    ...
    bool ComputeSplineIndex(const T& time_ns, size_t& idx, T& u) const
    {
        ...
        std::cout << std::fixed << std::setprecision(15) << "ComputeSplineIndex2 Problem :" << time_ns << " not in [" << segments[0].t0_ns << ", "
                  << segments[0].MaxTimeNs() << "]\n";
        return false;
    }
};
