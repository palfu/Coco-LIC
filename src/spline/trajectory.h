...
public:
    ...
    {
        this->extendKnotsTo(SO3d(Eigen::Quaterniond::Identity()), Eigen::Vector3d(0, 0, 0));
    ...
    void SetSensorExtrinsics(const SensorType type, const ExtrinsicParam &EP_StoI) { EP_StoI_[type] = EP_StoI; }

    const ExtrinsicParam &GetSensorEP(const SensorType type) const { return EP_StoI_.at(type); }
    ...
    void UpdateTimeOffset(const std::map<SensorType, double *> &t_offset_ns_params)
    {
    ...
    SE3d GetLidarPose(const double timestamp) const { return GetSensorPose(timestamp, EP_StoI_.at(LiDARSensor)); }

    /// [nurbs]
    SE3d GetIMUPoseNURBS(const double timestamp) const { return this->poseNsNURBS(timestamp * S_TO_NS); }

    SE3d GetIMUPoseNsNURBS(const int64_t timestamp) const { return this->poseNsNURBS(timestamp); }

    SE3d GetLidarPoseNURBS(const int64_t timestamp) const { return GetSensorPoseNURBS(timestamp, EP_StoI_.at(LiDARSensor)); }

    SE3d GetLidarPoseNURBS(const int64_t timestamp, int start_idx) const { return GetSensorPoseNURBS(timestamp, EP_StoI_.at(LiDARSensor), start_idx); }

    SE3d GetCameraPoseNURBS(const int64_t timestamp) const { return GetSensorPoseNURBS(timestamp, EP_StoI_.at(CameraSensor)); }

    Eigen::Vector3d GetTransVelWorldNURBS(const std::pair<int, double> su, double delta_t, const Eigen::Matrix4d blend_mat) const
    {
        return this->transVelWorldNURBS(su, delta_t, blend_mat);
    }

    Eigen::Vector3d GetPositionWorldNURBS(const double timestamp) const { return this->positionWorldNURBS(timestamp * S_TO_NS); }

    Eigen::Vector3d GetPositionWorldNsNURBS(const int64_t timestamp) const { return this->positionWorldNURBS(timestamp); }

    Eigen::Vector3d GetRotVelBodyNURBS(const double timestamp) const { return this->rotVelBodyNURBS(timestamp * S_TO_NS); }

    Eigen::Vector3d GetRotVelBodyNsNURBS(const int64_t timestamp) const { return this->rotVelBodyNURBS(timestamp); }

    SE3d GetCameraPose(const double timestamp) const { return GetSensorPose(timestamp, EP_StoI_.at(CameraSensor)); }

    // ================================================================ //
    void UndistortScan(const PosCloud &scan_raw, const int64_t target_timestamp, PosCloud &scan_in_target) const;

    void UndistortScanInG(const PosCloud &scan_raw, const int64_t scan_raw_timestamp, PosCloud &scan_in_target) const;
...
protected:
    ...
    SE3d GetSensorPose(const double timestamp, const ExtrinsicParam &EP_StoI) const;

    SE3d GetSensorPoseNURBS(const int64_t timestamp, const ExtrinsicParam &EP_StoI) const;

    SE3d GetSensorPoseNURBS(const int64_t timestamp, const ExtrinsicParam &EP_StoI, const int start_idx) const;
