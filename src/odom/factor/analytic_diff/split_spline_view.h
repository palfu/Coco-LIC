...
struct SplitSpineView : public So3SplineView, public RdSplineView {
    ...
    static SplineIMUData Evaluate(const int64_t time_ns,
                                  ...
                                  typename R3View::JacobianStruct* J_pos    = nullptr)
    {
        ...
        VecN lambda_a = splne_meta.pow_inv_dt[2] * R3View::blending_matrix_ * Up;

        ///
        VecN Ur = VecN::Zero();
        SO3View::template baseCoeffsWithTime<0>(Ur, u);
        VecN lambda_R = SO3View::blending_matrix_ * Ur;

        VecN Uw = VecN::Zero();
        SO3View::template baseCoeffsWithTime<1>(Uw, u);
        VecN lambda_w = splne_meta.pow_inv_dt[1] * SO3View::blending_matrix_ * Uw;
        ...
        if (J_rot_w) {
            ...
            for (int i = 1; i < DEG; i++) {
                d_omega_d_delta[i] = lambda_R[i + 1] * A_post_inv[i] * SO3::hat(omega[i]) * Jr_kdelta[i] + lambda_w[i + 1] * A_post_inv[i + 1];
            }

            ///
            for (int i = 0; i < DEG; i++) {
                J_rot_w->d_val_d_knot[i] -= d_omega_d_delta[i] * Jr_dvec_inv[i].transpose();
        ...
        if (J_rot_a) {
            ...
            ///
            Mat3 lhs = spline_imu_data.R_inv.matrix() * SO3::hat(accelerate + gravity);
    ...
    static SplineIMUData EvaluateNURBS(const int64_t time_ns,
                                       double const* const* knots,
                                       const Vec3& gravity,
                                       ...
                                       typename R3View::JacobianStruct* J_pos    = nullptr)
    {
        ...
        VecN Up = VecN::Zero();
        R3View::template baseCoeffsWithTimeR3<2>(Up, u);
        VecN lambda_a = 1.0 / (delta_t * delta_t) * blending_matrix * Up;

        ///
        VecN Ur = VecN::Zero();
        SO3View::template baseCoeffsWithTime<0>(Ur, u);
        VecN lambda_R = cumulative_blending_matrix * Ur;

        VecN Uw = VecN::Zero();
        SO3View::template baseCoeffsWithTime<1>(Uw, u);
        VecN lambda_w = 1.0 / delta_t * cumulative_blending_matrix * Uw;

        ///
        VecD accelerate = VecD::Zero();
        accelerate.setZero();
    ...
    static void EvaluatePhotoRpNURBS(const std::pair<int, double>& su,
                                     ...
                                     typename R3View::JacobianStruct* J_p  = nullptr)
    {  //d_pwb_d_pknot
        double u             = su.second;
        size_t s             = 0;
        size_t R_knot_offset = s;
