...
public:
    GravityFactor(const Eigen::Vector3d& gravity, const Eigen::Vector3d& sqrt_info) : gravity_(gravity), sqrt_info_(sqrt_info) {}
    ...
    virtual bool Evaluate(double const* const* parameters, double* residuals, double** jacobians) const
    {
        ...
        if (jacobians) {
            if (jacobians[0]) {
                Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor>> jac_grav(jacobians[0]);
                jac_grav.setZero();
                jac_grav.block<3, 3>(0, 0) = sqrt_info_.asDiagonal() * Eigen::Matrix3d::Identity();
            }
        }
...
public:
    ...
    virtual bool Evaluate(double const* const* parameters, double* residuals, double** jacobians) const
    {
        ...
        if (jacobians) {
            if (jacobians[0]) {
                Eigen::Map<Eigen::Matrix<double, 6, 3, Eigen::RowMajor>> jac_bg_i(jacobians[0]);
                jac_bg_i.setZero();
                jac_bg_i.block<3, 3>(0, 0) = -Eigen::Matrix3d::Identity();
                jac_bg_i.applyOnTheLeft(sqrt_info_);
            }
            if (jacobians[1]) {
                Eigen::Map<Eigen::Matrix<double, 6, 3, Eigen::RowMajor>> jac_bg_j(jacobians[1]);
                jac_bg_j.setZero();
                jac_bg_j.block<3, 3>(0, 0) = Eigen::Matrix3d::Identity();
                jac_bg_j.applyOnTheLeft(sqrt_info_);
            }
            if (jacobians[2]) {
                Eigen::Map<Eigen::Matrix<double, 6, 3, Eigen::RowMajor>> jac_ba_i(jacobians[2]);
                jac_ba_i.setZero();
                jac_ba_i.block<3, 3>(3, 0) = -Eigen::Matrix3d::Identity();
                jac_ba_i.applyOnTheLeft(sqrt_info_);
            }
            if (jacobians[3]) {
                Eigen::Map<Eigen::Matrix<double, 6, 3, Eigen::RowMajor>> jac_ba_j(jacobians[3]);
                jac_ba_j.setZero();
...
public:
    ...
    IMUFactor(int64_t time_ns, const IMUData& imu_data, const SplineSegmentMeta<SplineOrder>& spline_segment_meta, const Vec6d& info_vec)
        : time_ns_(time_ns)
        , imu_data_(imu_data)
    ...
    virtual bool Evaluate(double const* const* parameters, double* residuals, double** jacobians) const
    {
        ...
        if (jacobians) {
            spline_data = SplitView::Evaluate(t_corrected, spline_segment_meta_, parameters, gravity, &J_rot_w, &J_rot_a, &J_pos);
        } else {
            spline_data = SplitView::Evaluate(t_corrected, spline_segment_meta_, parameters, gravity);
        }

        Eigen::Map<Vec6d> residual(residuals);
        residual.block<3, 1>(0, 0) = spline_data.gyro - (imu_data_.gyro - gyro_bias);
        residual.block<3, 1>(3, 0) = spline_data.accel - (imu_data_.accel - accel_bias);
        ...
        if (jacobians) {
            for (size_t i = 0; i < knot_num; ++i) {
                if (jacobians[i]) {
                    Eigen::Map<Eigen::Matrix<double, 6, 4, Eigen::RowMajor>> jac_kont_R(jacobians[i]);
                    jac_kont_R.setZero();
                }
                if (jacobians[i + knot_num]) {
                    Eigen::Map<Eigen::Matrix<double, 6, 3, Eigen::RowMajor>> jac_kont_p(jacobians[i + knot_num]);
                    jac_kont_p.setZero();
                }
            }
        }
        ...
        for (size_t i = 0; i < SplineOrder; i++) {
            ...
            if (jacobians[idx]) {
                Eigen::Map<Eigen::Matrix<double, 6, 4, Eigen::RowMajor>> jac_knot_R(jacobians[idx]);
                jac_knot_R.setZero();
                /// for gyro residual
        ...
        for (size_t i = 0; i < SplineOrder; i++) {
            ...
            if (jacobians[idx]) {
                Eigen::Map<Eigen::Matrix<double, 6, 3, Eigen::RowMajor>> jac_knot_p(jacobians[idx]);
                jac_knot_p.setZero();

                /// for accel residual
                jac_knot_p.block<3, 3>(3, 0) = J_pos.d_val_d_knot[i] * spline_data.R_inv.matrix();
        ...
        if (jacobians[2 * knot_num]) {
            Eigen::Map<Eigen::Matrix<double, 6, 3, Eigen::RowMajor>> jac_bw(jacobians[2 * knot_num]);
            jac_bw.setZero();
            jac_bw.block<3, 3>(0, 0).diagonal() = info_vec_.head(3);
        }

        if (jacobians[2 * knot_num + 1]) {
            Eigen::Map<Eigen::Matrix<double, 6, 3, Eigen::RowMajor>> jac_ba(jacobians[2 * knot_num + 1]);
            jac_ba.setZero();
        ...
        if (jacobians[2 * knot_num + 2]) {
            Eigen::Map<Eigen::Matrix<double, 6, 3, Eigen::RowMajor>> jac_gravity(jacobians[2 * knot_num + 2]);
            jac_gravity.setZero();
        ...
        if (jacobians[2 * knot_num + 3]) {
            Eigen::Map<Eigen::Matrix<double, 6, 1>> jac_t_offset(jacobians[2 * knot_num + 3]);
            jac_t_offset.setZero();

            Eigen::Vector3d rot_accel = SO3View::accelerationBody(t_corrected, spline_segment_meta_, parameters);
            Eigen::Vector3d jerk_R3   = R3View::evaluate<3>(t_corrected, spline_segment_meta_, parameters + knot_num);
            ...
            jac_t_offset.block<3, 1>(0, 0) =
                SO3d::vee(rot_dot.transpose() * rot_dot) + SO3d::vee(rot.transpose() * (rot_dot * gyro_hat + rot * SO3d::hat(rot_accel)));
            jac_t_offset.block<3, 1>(3, 0) = rot_dot.transpose() * rot * spline_data.accel + rot.transpose() * jerk_R3;
...
public:
    ...
    IMUFactorNURBS(int64_t time_ns,
    ...
    virtual bool Evaluate(double const* const* parameters, double* residuals, double** jacobians) const
    {
        ...
        if (jacobians) {
            spline_data =
                SplitView::EvaluateNURBS(time_ns_, parameters, gravity_, knts_, su_, blending_matrix_, cumulative_blending_matrix_, &J_rot_w, &J_rot_a, &J_pos);
        } else {
            spline_data = SplitView::EvaluateNURBS(time_ns_, parameters, gravity_, knts_, su_, blending_matrix_, cumulative_blending_matrix_);
        }
...
public:
    IMUPoseFactor(int64_t time_ns,
                  const PoseData& pose_data,
                  const SplineSegmentMeta<SplineOrder>& spline_segment_meta,
                  const Eigen::Matrix<double, 6, 1>& info_vec)
        : time_ns_(time_ns), pose_data_(pose_data), spline_segment_meta_(spline_segment_meta), info_vec_(info_vec)
    {
        ///
        set_num_residuals(6);
        ///
    ...
    virtual bool Evaluate(double const* const* parameters, double* residuals, double** jacobians) const
    {
        ...
        if (jacobians) {
            S_ItoG = so3_spline_view.EvaluateRotation(t_corrected, spline_segment_meta_, parameters, &J_R);
            p_IinG = r3_spline_view.evaluate(t_corrected, spline_segment_meta_, parameters + P_offset, &J_p);
        } else {
            S_ItoG = so3_spline_view.EvaluateRotation(t_corrected, spline_segment_meta_, parameters);
            p_IinG = r3_spline_view.evaluate(t_corrected, spline_segment_meta_, parameters + P_offset);
        }
        ...
        if (jacobians) {
            ...
            for (size_t i = 0; i < SplineOrder; i++) {
                ...
                if (jacobians[idx]) {
                    Eigen::Map<Eigen::Matrix<double, 6, 4, Eigen::RowMajor>> jacobian_kont_R(jacobians[idx]);
            ...
            for (size_t i = 0; i < SplineOrder; i++) {
                ...
                if (jacobians[idx + knot_num]) {
                    Eigen::Map<Eigen::Matrix<double, 6, 3, Eigen::RowMajor>> jacobian_kont_P(jacobians[idx + knot_num]);
                    jacobian_kont_P.setZero();

                    /// for position residual
                    jacobian_kont_P.block<3, 3>(3, 0) = J_p.d_val_d_knot[i] * Eigen::Matrix3d::Identity();
                    /// L*J
            ...
            // Time_offset
            if (jacobians[2 * knot_num]) {
                Eigen::Map<Eigen::Matrix<double, 6, 1>> jac_t_offset(jacobians[2 * knot_num]);
                jac_t_offset.setZero();

                Eigen::Vector3d gyro = So3SplineView::VelocityBody(t_corrected, spline_segment_meta_, parameters);
                Eigen::Vector3d vel  = RdSplineView::velocity(t_corrected, spline_segment_meta_, parameters + knot_num);

                Eigen::Matrix3d gyro_hat       = SO3d::hat(gyro);
                SO3d rot_dot                   = S_ItoG * SO3d(Eigen::Quaterniond(gyro_hat).normalized());
                jac_t_offset.block<3, 1>(0, 0) = (rot_dot * pose_data_.orientation.inverse()).log();  //
...
public:
    ...
    IMUPoseFactorNURBS(int64_t time_ns,
                       const PoseData& pose_data,
                       const Vec6d& info_vec,
                       const std::vector<int64_t>& knts,
                       const std::pair<int, double>& su,
    ...
    virtual bool Evaluate(double const* const* parameters, double* residuals, double** jacobians) const
    {
        ...
        Eigen::Vector3d p_IinG = Eigen::Vector3d::Zero();
        if (jacobians) {
            S_ItoG = so3_spline_view.EvaluateRotationNURBS(su_, cumulative_blending_matrix_, parameters, &J_R);
            p_IinG = r3_spline_view.evaluateNURBS(su_, blending_matrix_, parameters + P_offset, &J_p);
        } else {
            S_ItoG = so3_spline_view.EvaluateRotationNURBS(su_, cumulative_blending_matrix_, parameters);
            p_IinG = r3_spline_view.evaluateNURBS(su_, blending_matrix_, parameters + P_offset);
        }
...
public:
    ...
    IMURelativePoseFactorNURBS(int64_t time_ns,
                               const PoseData& pose_data,
        ...
        if (jacobians) {
            ...
            for (size_t i = 0; i < SplineOrder; i++) {
                ...
                if (jacobians[idx + knot_num]) {
                    ...
                    jacobian_kont_P.block<3, 3>(3, 0) = J_p.d_val_d_knot[i] * rot_init.inverse().matrix() * Eigen::Matrix3d::Identity();  //
                    /// L*J
                    jacobian_kont_P = (info_vec_.asDiagonal() * jacobian_kont_P).eval();
                }
            }
...
class LocalVelocityFactor : public ceres::CostFunction, So3SplineView, RdSplineView {
public:
    using SO3View = So3SplineView;
    using R3View  = RdSplineView;
    ...
    LocalVelocityFactor(int64_t time_ns, const Eigen::Vector3d& local_velocity, const SplineSegmentMeta<SplineOrder>& spline_segment_meta, double weight)
        : time_ns_(time_ns), local_velocity_(local_velocity), spline_segment_meta_(spline_segment_meta), weight_(weight)
    {
        ///
        set_num_residuals(3);
    ...
    virtual bool Evaluate(double const* const* parameters, double* residuals, double** jacobians) const
    {
        ...
        if (jacobians) {
            S_ItoG = SO3View::EvaluateRp(t_corrected, spline_segment_meta_, parameters, &J_R);
            v_inG  = R3View::velocity(t_corrected, spline_segment_meta_, parameters + knot_num, &J_p);
        } else {
            S_ItoG = SO3View::EvaluateRp(t_corrected, spline_segment_meta_, parameters, nullptr);
            v_inG  = R3View::velocity(t_corrected, spline_segment_meta_, parameters + knot_num, nullptr);
        }
        ...
        if (jacobians) {
            for (size_t i = 0; i < knot_num; ++i) {
                if (jacobians[i]) {
                    Eigen::Map<Eigen::Matrix<double, 3, 4, Eigen::RowMajor>> jac_kont_R(jacobians[i]);
                    jac_kont_R.setZero();
                }
                if (jacobians[i + knot_num]) {
                    Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor>> jac_kont_p(jacobians[i + knot_num]);
                    jac_kont_p.setZero();
                }
            }
        }
        ...
        for (size_t i = 0; i < SplineOrder; i++) {
            ...
            if (jacobians[idx]) {
                Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor>> jac_kont_p(jacobians[idx]);
                jac_kont_p.setZero();
        ...
        // Time_offset
        if (jacobians[2 * knot_num]) {
            Eigen::Map<Eigen::Matrix<double, 3, 1>> jac_t_offset(jacobians[2 * knot_num]);
            jac_t_offset.setZero();

            Eigen::Vector3d gyro  = So3SplineView::VelocityBody(t_corrected, spline_segment_meta_, parameters);
            Eigen::Vector3d accel = RdSplineView::acceleration(t_corrected, spline_segment_meta_, parameters + knot_num);
...
class Local6DoFVelocityFactor : public ceres::CostFunction, So3SplineView, RdSplineView {
public:
    using SO3View = So3SplineView;
    using R3View  = RdSplineView;
    ...
    Local6DoFVelocityFactor(int64_t time_ns,
                            const Eigen::Matrix<double, 6, 1>& local_velocity,
                            const SplineSegmentMeta<SplineOrder>& spline_segment_meta,
                            const Eigen::Matrix<double, 6, 1>& sqrt_info)
        : time_ns_(time_ns), local_velocity_(local_velocity), spline_segment_meta_(spline_segment_meta), sqrt_info_(sqrt_info)
    {
        ///
        set_num_residuals(6);
    ...
    virtual bool Evaluate(double const* const* parameters, double* residuals, double** jacobians) const { return false; }
...
public:
    ...
    RelativeOrientationFactor(const SO3d& S_BtoA, int64_t ta_ns, int64_t tb_ns, const SplineMeta<SplineOrder>& spline_meta, const Eigen::Vector3d& info_vec)
        : S_BtoA_(S_BtoA), ta_ns_(ta_ns), tb_ns_(tb_ns), spline_meta_(spline_meta)
    {
        ///
        set_num_residuals(3);
    ...
    virtual bool Evaluate(double const* const* parameters, double* residuals, double** jacobians) const
    {
        ...
        if (jacobians) {
            S_IatoG = SO3View::EvaluateRotation(ta_ns_, spline_meta_.segments.at(seg_idx[0]), parameters + R_offset[0], &J_R[0]);
            S_GtoIb = SO3View::EvaluateLogRT(tb_ns_, spline_meta_.segments.at(seg_idx[1]), parameters + R_offset[1], &J_R[1]);
        } else {
            S_IatoG = SO3View::EvaluateRotation(ta_ns_, spline_meta_.segments.at(seg_idx[0]), parameters + R_offset[0], nullptr);
            S_GtoIb = SO3View::EvaluateLogRT(tb_ns_, spline_meta_.segments.at(seg_idx[1]), parameters + R_offset[1], nullptr);
        }
            ...
            for (int seg = 0; seg < 2; ++seg) {
                for (size_t i = 0; i < SplineOrder; i++) {
                    ...
                    if (jacobians[idx]) {
                        Eigen::Map<Eigen::Matrix<double, 3, 4, Eigen::RowMajor>> jac_kont_R(jacobians[idx]);
                        // 3*3 3*3
                        jac_kont_R.block<3, 3>(0, 0) += sqrt_info * jac_lhs_dR_dR[seg] * J_R[seg].d_val_d_knot[i];
                    }
                }
            }
        }
