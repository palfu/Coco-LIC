...
bool MarginalizationInfo::marginalize()
{
  ...
  LOG(INFO) << "Amm_S cnt: " << cnt[0] << "/" << cnt[1] << "; n/m " << n << "/" << m << "; " << ss.str();
    ...
    time[4] = timer.toc();
    LOG(INFO) << "marginalize costs " << time[0] << "/" << time[1] - time[0] << "/" << time[2] - time[1] << "/" << time[3] - time[2] << "/" << time[4] - time[3]
              << " = " << time[4] << " ms.";
...
MarginalizationFactor::MarginalizationFactor(MarginalizationInfo::Ptr &_marginalization_info) : marginalization_info(_marginalization_info.get())
{
    int cnt = 0;
    ///
