...
class LoamFeatureFactor : public ceres::CostFunction, So3SplineView, RdSplineView {
public:
    ...
    LoamFeatureFactor(int64_t t_point_ns,
    ...
    virtual bool Evaluate(double const* const* parameters, double* residuals, double** jacobians) const
    {
        ...
        size_t kont_num = spline_segment_meta_.NumParameters();
        if (jacobians) {
            S_ItoG = SO3View::EvaluateRp(t_point_ns_, spline_segment_meta_, parameters, &J_R);
            p_IinG = R3View::evaluate(t_point_ns_, spline_segment_meta_, parameters + kont_num, &J_p);
        } else {
            S_ItoG = SO3View::EvaluateRp(t_point_ns_, spline_segment_meta_, parameters, nullptr);
            p_IinG = R3View::evaluate(t_point_ns_, spline_segment_meta_, parameters + kont_num, nullptr);
        }
...
class LoamFeatureFactorNURBS : public ceres::CostFunction, So3SplineView, RdSplineView {
public:
    using SO3View = So3SplineView;
    using R3View  = RdSplineView;
    ...
    LoamFeatureFactorNURBS(int64_t t_point_ns,
                           const PointCorrespondence& pc,
    ...
    virtual bool Evaluate(double const* const* parameters, double* residuals, double** jacobians) const
    {
        ...
        if (jacobians) {
            S_ItoG = SO3View::EvaluateRpNURBS(su_, cumulative_blending_matrix_, parameters, &J_R);
            p_IinG = R3View::evaluateNURBS(su_, blending_matrix_, parameters + 4, &J_p);
        } else {
            S_ItoG = SO3View::EvaluateRpNURBS(su_, cumulative_blending_matrix_, parameters, nullptr);
            p_IinG = R3View::evaluateNURBS(su_, blending_matrix_, parameters + 4, nullptr);
        }
        Vec3d p_M = S_GtoM_ * (S_ItoG * p_IK + p_IinG) + p_GinM_;
        ...
        if (jacobians) {
            for (size_t i = 0; i < 4; ++i) {
                ...
                if (jacobians[i + 4]) {
                    Eigen::Map<Eigen::Matrix<double, 1, 3, Eigen::RowMajor>> jac_kont_p(jacobians[i + 4]);
                    jac_kont_p.setZero();
                }
            }
        }
...
class LoamFeatureOptMapPoseFactor : public ceres::CostFunction, So3SplineView, RdSplineView {
public:
    using SO3View = So3SplineView;
    using R3View  = RdSplineView;
    ...
    LoamFeatureOptMapPoseFactor(int64_t t_point_ns, const PointCorrespondence& pc, const SplineSegmentMeta<SplineOrder>& spline_segment_meta, double weight)
        : t_point_ns_(t_point_ns), pc_(pc), spline_segment_meta_(spline_segment_meta), weight_(weight)
    {
    ...
    virtual bool Evaluate(double const* const* parameters, double* residuals, double** jacobians) const
    {
        ...
        if (jacobians) {
            S_IktoG = SO3View::EvaluateRp(t_point_ns_, spline_segment_meta_, parameters, &J_R);
            p_IkinG = R3View::evaluate(t_point_ns_, spline_segment_meta_, parameters + kont_num, &J_p);
        } else {
            S_IktoG = SO3View::EvaluateRp(t_point_ns_, spline_segment_meta_, parameters, nullptr);
            p_IkinG = R3View::evaluate(t_point_ns_, spline_segment_meta_, parameters + kont_num, nullptr);
        }
        ...
        if (jacobians[kont_num * 2]) {
            Eigen::Map<Eigen::Matrix<double, 1, 4, Eigen::RowMajor>> jac_Map_R(jacobians[kont_num * 2]);
            jac_Map_R.setZero();

            Vec3d p_Im                  = S_ImtoG.inverse() * (p_G - p_IminG);
            jac_Map_R.block<1, 3>(0, 0) = J_pi.transpose() * (S_ItoL.matrix() * SO3d::hat(p_Im));
            jac_Map_R                   = (weight_ * jac_Map_R).eval();
        }
        // map position
        if (jacobians[kont_num * 2 + 1]) {
            Eigen::Map<Eigen::Matrix<double, 1, 3, Eigen::RowMajor>> jac_Map_p(jacobians[kont_num * 2 + 1]);
            jac_Map_p.setZero();
...
class RalativeLoamFeatureFactor : public ceres::CostFunction, So3SplineView, RdSplineView {
public:
    using SO3View = So3SplineView;
    using R3View  = RdSplineView;
    ...
    RalativeLoamFeatureFactor(int64_t t_point_ns, int64_t t_map_ns, const PointCorrespondence& pc, const SplineMeta<SplineOrder>& spline_meta, double weight)
        : t_point_ns_(t_point_ns), t_map_ns_(t_map_ns), pc_(pc), spline_meta_(spline_meta), weight_(weight)
    {
    ...
    virtual bool Evaluate(double const* const* parameters, double* residuals, double** jacobians) const
    {
        ...
        if (jacobians) {
            // rhs = p_Ii
            S_IitoG = SO3View::EvaluateRp(t_i_, spline_meta_.segments.at(seg_idx[0]), parameters + R_offset[0], &J_R[0]);
            p_IiinG = R3View::evaluate(t_i_, spline_meta_.segments.at(seg_idx[0]), parameters + P_offset[0], &J_p[0]);
        } else {
            S_IitoG = SO3View::EvaluateRp(t_i_, spline_meta_.segments.at(seg_idx[0]), parameters + R_offset[0], nullptr);
            p_IiinG = R3View::evaluate(t_i_, spline_meta_.segments.at(seg_idx[0]), parameters + P_offset[0], nullptr);
        }
        ///
        ...
        if (jacobians) {
            // rhs = p_G - p_IjinG
            S_GtoIj = SO3View::EvaluateRTp(t_j_, spline_meta_.segments.at(seg_idx[1]), parameters + R_offset[1], &J_R[1]);
            p_IjinG = R3View::evaluate(t_j_, spline_meta_.segments.at(seg_idx[1]), parameters + P_offset[1], &J_p[1]);
        } else {
            S_GtoIj = SO3View::EvaluateRTp(t_j_, spline_meta_.segments.at(seg_idx[1]), parameters + R_offset[1], nullptr);
            p_IjinG = R3View::evaluate(t_j_, spline_meta_.segments.at(seg_idx[1]), parameters + P_offset[1], nullptr);
        }
        ...
        if (jacobians) {
            ...
            //
            jac_lhs_R[0] = -J_pi.transpose() * (S_GtoCj * S_IitoG).matrix() * SO3::hat(p_Ii);
            jac_lhs_P[0] = J_pi.transpose() * S_GtoCj.matrix();

            //
            jac_lhs_R[1] = J_pi.transpose() * S_GtoCj.matrix() * SO3::hat(p_G - p_IjinG);
