...
class PnPFactorNURBS : public ceres::CostFunction, So3SplineView, RdSplineView {
public:
    ...
    PnPFactorNURBS(const int64_t t_img,
    ...
    virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
    {
        ...
        if (jacobians) {
            SplitView::EvaluatePhotoRpNURBS(su_, cumulative_blending_matrix_, blending_matrix_, parameters, S_VtoI_, p_VinI_, v_point_, &S_ItoG, &p_IinG, &J_R,
                                            &J_p);
        } else {
            SplitView::EvaluatePhotoRpNURBS(su_, cumulative_blending_matrix_, blending_matrix_, parameters, S_VtoI_, p_VinI_, v_point_, &S_ItoG, &p_IinG,
                                            nullptr, nullptr);
        }
        ...
        if (jacobians) {
            for (size_t i = 0; i < 4; ++i) {
                ...
                if (jacobians[i + 4]) {
                    Eigen::Map<Eigen::Matrix<double, 2, 3, Eigen::RowMajor>> jac_kont_p(jacobians[i + 4]);
                    jac_kont_p.setZero();
                }
            }
        }
        ...
        //     0, fy / Z, -fy * Y / (Z * Z);
        d_uv_d_pC << -fx / Z, 0, fx * X / (Z * Z), 0, -fy / Z, fy * Y / (Z * Z);
...
class PhotometricFactorNURBS : public ceres::CostFunction, So3SplineView, RdSplineView {
public:
    using SO3View   = So3SplineView;
    using R3View    = RdSplineView;
    using SplitView = SplitSpineView;
    ...
    PhotometricFactorNURBS(const int patch_size_half,
                           const int scale,
                           int level,
                           const int64_t t_img,
                           const std::pair<int, double> &su,
    ...
    virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
    {
        ...
        for (int x = 0; x < patch_size_; x++) {
            uint8_t *img_ptr = (uint8_t *) cur_img_.data + (v_i + x * scale_ - patch_size_half_ * scale_) * cur_img_.cols + u_i - patch_size_half_ * scale_;
            for (int y = 0; y < patch_size_; ++y, img_ptr += scale_) {
                double res =
                    patch_[patch_size_total * level_ + x * patch_size_ + y] -
                    (w_tl * img_ptr[0] + w_tr * img_ptr[scale_] + w_bl * img_ptr[scale_ * cur_img_.cols] + w_br * img_ptr[scale_ * cur_img_.cols + scale_]);
                double du =
                    0.5 *
                    ((w_tl * img_ptr[scale_] + w_tr * img_ptr[scale_ * 2] + w_bl * img_ptr[scale_ * cur_img_.cols + scale_] +
                      w_br * img_ptr[scale_ * cur_img_.cols + scale_ * 2]) -
                     (w_tl * img_ptr[-scale_] + w_tr * img_ptr[0] + w_bl * img_ptr[scale_ * cur_img_.cols - scale_] + w_br * img_ptr[scale_ * cur_img_.cols])) *
                    (1.0 / scale_);
                double dv = 0.5 *
                            ((w_tl * img_ptr[scale_ * cur_img_.cols] + w_tr * img_ptr[scale_ + scale_ * cur_img_.cols] +
                              w_bl * img_ptr[cur_img_.cols * scale_ * 2] + w_br * img_ptr[cur_img_.cols * scale_ * 2 + scale_]) -
                             (w_tl * img_ptr[-scale_ * cur_img_.cols] + w_tr * img_ptr[-scale_ * cur_img_.cols + scale_] + w_bl * img_ptr[0] +
                              w_br * img_ptr[scale_])) *
                            (1.0 / scale_);
        ...
        if (jacobians) {
            for (size_t i = 0; i < 4; ++i) {
                if (jacobians[i]) {
                    ...
                    //     jacobians[i]);
                    Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> jac_kont_R(jacobians[i], patch_size_ * patch_size_, 4);
                    jac_kont_R.setZero();
                }
                if (jacobians[i + 4]) {
                    ...
                    //     jacobians[i + 4]);
                    Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> jac_kont_p(jacobians[i + 4], patch_size_ * patch_size_,
                                                                                                                  3);
                    jac_kont_p.setZero();
                }
            }
        ...
        for (size_t i = 0; i < 4; i++) {
            ...
            if (jacobians[idx]) {
                ...
                // Eigen::Map<Eigen::Matrix<double, 64, 4, Eigen::RowMajor>> jac_kont_R(
                //     jacobians[idx]);
                Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> jac_kont_R(jacobians[idx], patch_size_ * patch_size_, 4);
                jac_kont_R.setZero();
        ...
        for (size_t i = 0; i < 4; i++) {
            ...
            if (jacobians[idx]) {
                ...
                // Eigen::Map<Eigen::Matrix<double, 64, 3, Eigen::RowMajor>> jac_kont_p(
                //     jacobians[idx]);
                Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> jac_kont_p(jacobians[idx], patch_size_ * patch_size_, 3);
                jac_kont_p.setZero();
...
private:
    inline double GetPixelValue(double u, double v) const
    {
        ...
        return double((1 - uu) * (1 - vv) * data[0] + uu * (1 - vv) * data[1] + (1 - uu) * vv * data[cur_img_.step] + uu * vv * data[cur_img_.step + 1]);
    }
    ...
    inline double GetPyramidIntPixelValue(int u_i, int v_i, double w_tl, double w_tr, double w_bl, double w_br) const
    {
        ...
        return double(w_tl * data[0] + w_tr * data[scale_] + w_bl * data[scale_ * cur_img_.step] + w_br * data[scale_ * cur_img_.step + scale_]);
    }
...
///
class PhotometricFactorNURBS2 : public ceres::CostFunction, So3SplineView, RdSplineView {
public:
    using SO3View   = So3SplineView;
    using R3View    = RdSplineView;
    using SplitView = SplitSpineView;
    ...
    PhotometricFactorNURBS2(const int64_t t_img,
                            const std::pair<int, double> &su,
                            const Eigen::Matrix4d &blending_matrix,
                            const Eigen::Matrix4d &cumulative_blending_matrix,
                            const double &prev_pixel_intensity,
...
class ImageFeatureFactor : public ceres::CostFunction, So3SplineView, RdSplineView {
public:
    using SO3View = So3SplineView;
    using R3View  = RdSplineView;
    ...
    ImageFeatureFactor(
        const int64_t t_i_ns, const Eigen::Vector3d &p_i, const int64_t t_j_ns, const Eigen::Vector3d &p_j, const SplineMeta<SplineOrder> &spline_meta)
        : t_i_ns_(t_i_ns), p_i_(p_i), t_j_ns_(t_j_ns), p_j_(p_j), spline_meta_(spline_meta)
    {
    ...
    virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
    {
        ...
        if (jacobians) {
            // rhs = p_Ii
            S_IitoG = SO3View::EvaluateRp(ti_corrected_ns, spline_meta_.segments.at(seg_idx[0]), parameters + R_offset[0], &J_R[0]);
            p_IiinG = R3View::evaluate(ti_corrected_ns, spline_meta_.segments.at(seg_idx[0]), parameters + P_offset[0], &J_p[0]);
        } else {
            S_IitoG = SO3View::EvaluateRp(ti_corrected_ns, spline_meta_.segments.at(seg_idx[0]), parameters + R_offset[0], nullptr);
            p_IiinG = R3View::evaluate(ti_corrected_ns, spline_meta_.segments.at(seg_idx[0]), parameters + P_offset[0], nullptr);
        }
        ///
        ...
        if (jacobians) {
            // rhs = p_G - p_IjinG
            S_GtoIj = SO3View::EvaluateRTp(tj_corrected_ns, spline_meta_.segments.at(seg_idx[1]), parameters + R_offset[1], &J_R[1]);
            p_IjinG = R3View::evaluate(tj_corrected_ns, spline_meta_.segments.at(seg_idx[1]), parameters + P_offset[1], &J_p[1]);
        } else {
            S_GtoIj = SO3View::EvaluateRTp(tj_corrected_ns, spline_meta_.segments.at(seg_idx[1]), parameters + R_offset[1], nullptr);
            p_IjinG = R3View::evaluate(tj_corrected_ns, spline_meta_.segments.at(seg_idx[1]), parameters + P_offset[1], nullptr);
        }

        Vec3d gyro_i, gyro_j;
        Vec3d vel_i, vel_j;
        if (jacobians && jacobians[Knot_offset + 1]) {
            gyro_i = SO3View::VelocityBody(ti_corrected_ns, spline_meta_.segments.at(seg_idx[0]), parameters + R_offset[0]);
            vel_i  = R3View::velocity(ti_corrected_ns, spline_meta_.segments.at(seg_idx[0]), parameters + P_offset[0]);

            gyro_j = SO3View::VelocityBody(tj_corrected_ns, spline_meta_.segments.at(seg_idx[1]), parameters + R_offset[1]);
            vel_j  = R3View::velocity(tj_corrected_ns, spline_meta_.segments.at(seg_idx[1]), parameters + P_offset[1]);
        }
        ...
        if (jacobians) {
            ...
            if (jacobians[Knot_offset + 1]) {
                Eigen::Map<Eigen::Matrix<double, 2, 1>> jac_t_offset(jacobians[Knot_offset + 1]);
                jac_t_offset.setZero();

                Mat3d Ri_dot = S_IitoG.matrix() * SO3d::hat(gyro_i);
                Mat3d Rj_dot = S_GtoIj.inverse().matrix() * SO3d::hat(gyro_j);
                // t_j
                Vec3d J_tj = S_ItoC.matrix() * Rj_dot.transpose() * (p_G - p_IjinG) - S_GtoCj * vel_j;
                // t_i
...
class Image3D2DFactor : public ceres::CostFunction, So3SplineView, RdSplineView {
public:
    using SO3View = So3SplineView;
    using R3View  = RdSplineView;
    ...
    Image3D2DFactor(const int64_t t_j_ns, const Eigen::Vector3d &p_j, const SplineMeta<SplineOrder> &spline_meta)
    ...
    virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
    {
        ...
        if (jacobians) {
            // rhs = p_G - p_IjinG
            S_GtoIj = SO3View::EvaluateRTp(tj_corrected_ns, spline_meta_.segments.at(0), parameters, &J_R);
            p_IjinG = R3View::evaluate(tj_corrected_ns, spline_meta_.segments.at(0), parameters + kont_num, &J_p);
        } else {
            S_GtoIj = SO3View::EvaluateRTp(tj_corrected_ns, spline_meta_.segments.at(0), parameters, nullptr);
            p_IjinG = R3View::evaluate(tj_corrected_ns, spline_meta_.segments.at(0), parameters + kont_num, nullptr);
        }

        Vec3d gyro_j, vel_j;
        if (jacobians && jacobians[Knot_offset + 1]) {
            gyro_j = SO3View::VelocityBody(tj_corrected_ns, spline_meta_.segments.at(0), parameters);
            vel_j  = R3View::velocity(tj_corrected_ns, spline_meta_.segments.at(0), parameters + kont_num);
        }
        ...
        if (jacobians) {
            ...
            /// jacobian of p_G
            if (jacobians[Knot_offset]) {
                Eigen::Map<Eigen::Matrix<double, 2, 3, Eigen::RowMajor>> jac_p_G(jacobians[Knot_offset]);
                jac_p_G.setZero();
...
class ImageFeatureOnePoseFactor : public ceres::CostFunction, So3SplineView, RdSplineView {
public:
    using SO3View = So3SplineView;
    using R3View  = RdSplineView;
    ...
    ImageFeatureOnePoseFactor(const Eigen::Vector3d &p_i,
                              const SO3d &S_IitoG,
                              const Eigen::Vector3d &p_IiinG,
                              const int64_t t_j_ns,
                              const Eigen::Vector3d &p_j,
                              const SplineMeta<SplineOrder> &spline_meta)
        : p_i_(p_i), S_IitoG_(S_IitoG), p_IiinG_(p_IiinG), t_j_ns_(t_j_ns), p_j_(p_j), spline_meta_(spline_meta)
    {
    ...
    virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
    {
        ...
        if (jacobians) {
            // rhs = p_G - p_IjinG
            S_GtoIj = SO3View::EvaluateRTp(t_j_ns_, spline_meta_.segments.at(0), parameters + R_offset, &J_R);
            p_IjinG = R3View::evaluate(t_j_ns_, spline_meta_.segments.at(0), parameters + P_offset, &J_p);
        } else {
            S_GtoIj = SO3View::EvaluateRTp(t_j_ns_, spline_meta_.segments.at(0), parameters + R_offset, nullptr);
            p_IjinG = R3View::evaluate(t_j_ns_, spline_meta_.segments.at(0), parameters + P_offset, nullptr);
        }
        SO3d S_ItoC  = S_CtoI.inverse();
        SO3d S_GtoCj = S_ItoC * S_GtoIj;
        Vec3d x_j    = S_GtoCj * (p_G - p_IjinG) - S_ItoC * p_CinI;
...
public:
    ...
    ImageDepthFactor(const Eigen::Vector3d &p_i, const Eigen::Vector3d &p_j, const SO3d &S_CitoCj, const Eigen::Vector3d &p_CiinCj)
...
public:
    ...
    EpipolarFactor(const int64_t t_i_ns,
                   ...
                   const SplineMeta<SplineOrder> &spline_meta,
                   double weight)
        : t_i_ns_(t_i_ns), x_i_(x_i), x_k_(x_k), S_GtoCk_(S_GtoCk), p_CkinG_(p_CkinG), spline_meta_(spline_meta), weight_(weight)
    {
    ...
    virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
    {
        ...
        Vec3d p_IiinG = Vec3d::Zero();
        if (jacobians) {
            S_IitoG = SO3View::EvaluateRp(t_i_ns_, spline_meta_.segments[0], parameters, &J_R);
            p_IiinG = R3View::evaluate(t_i_ns_, spline_meta_.segments[0], parameters + kont_num, &J_p);
        } else {
            S_IitoG = SO3View::EvaluateRp(t_i_ns_, spline_meta_.segments[0], parameters, nullptr);
            p_IiinG = R3View::evaluate(t_i_ns_, spline_meta_.segments[0], parameters + kont_num, nullptr);
        }

        //
        Vec3d Rxi             = S_GtoCk_ * S_IitoG * S_CtoI * x_i_;
        Eigen::Matrix3d t_hat = SO3::hat(S_GtoCk_ * (S_IitoG * p_CinI + p_IiinG - p_CkinG_));
        ...
        if (jacobians) {
            for (size_t i = 0; i < kont_num; ++i) {
                if (jacobians[i]) {
                    Eigen::Map<Eigen::Matrix<double, 1, 4, Eigen::RowMajor>> jac_kont_R(jacobians[i]);
                    jac_kont_R.setZero();
                }
                if (jacobians[i + kont_num]) {
                    Eigen::Map<Eigen::Matrix<double, 1, 3, Eigen::RowMajor>> jac_kont_p(jacobians[i + kont_num]);
                    jac_kont_p.setZero();
                }
            }
            ...
            Vec3d jac_lhs_R = -x_k_.transpose() * t_hat * (S_GtoCk_ * S_IitoG).matrix() * SO3::hat(S_CtoI * x_i_);
            ...
            for (size_t i = 0; i < kont_num; i++) {
                size_t idx = i;
                if (jacobians[idx]) {
                    Eigen::Map<Eigen::Matrix<double, 1, 4, Eigen::RowMajor>> jac_kont_R(jacobians[idx]);
                    jac_kont_R.setZero();

                    /// 1*3 3*3
                    jac_kont_R.block<1, 3>(0, 0) = jac_lhs_R.transpose() * J_R.d_val_d_knot[i];
