...
public:
    ...
    static VecD evaluate(const int64_t time_ns, const SplineSegmentMeta<N>& splne_meta, double const* const* knots, JacobianStruct* J = nullptr)
    {
    ...
    template <int Derivative = 0>
    static VecD evaluateNURBS(const std::pair<int, double>& su, const Eigen::Matrix4d& blending_matrix, double const* const* knots, JacobianStruct* J = nullptr)
    {
        double u = su.second;
    ...
    static VecD velocity(const int64_t time_ns, const SplineSegmentMeta<N>& splne_meta, double const* const* knots, JacobianStruct* J = nullptr)
    {
        return evaluate<1>(time_ns, splne_meta, knots, J);
    }

    /// @brief Alias for second derivative of spline. See \ref evaluate.
    static VecD acceleration(const int64_t time_ns, const SplineSegmentMeta<N>& splne_meta, double const* const* knots, JacobianStruct* J = nullptr)
    {
    ...
    static void baseCoeffsWithTimeR3(const Eigen::MatrixBase<Derived>& res_const, double t)
    {
