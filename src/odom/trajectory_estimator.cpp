...
TrajectoryEstimator::TrajectoryEstimator(Trajectory::Ptr trajectory, TrajectoryEstimatorOptions &option, std::string descri)
    : trajectory_(trajectory), fixed_control_point_index_(-1)
{
    ...
    // For gravity
    homo_vec_local_parameterization_ = new ceres::HomogeneousVectorParameterization(3);
...
void TrajectoryEstimator::AddControlPointsNURBS(size_t start_idx, std::vector<double *> &vec, bool addPosKnot)
{
    for (size_t i = start_idx; i < start_idx + 4; ++i) {
        ...
        if (options.lock_traj || (fixed_control_point_index_ >= 0 && i <= size_t(fixed_control_point_index_))) {
            problem_->SetParameterBlockConstant(vec.back());
        }
    }
}

void TrajectoryEstimator::AddControlPointsNURBS(size_t start_idx1, size_t start_idx2, std::vector<double *> &vec, bool addPosKnot)
{
...
void TrajectoryEstimator::AddMarginalizationFactor(MarginalizationInfo::Ptr &last_marginalization_info,
                                                   std::vector<double *> &last_marginalization_parameter_blocks)
{
    MarginalizationFactor *marginalization_factor = new MarginalizationFactor(last_marginalization_info);
    problem_->AddResidualBlock(marginalization_factor, NULL, last_marginalization_parameter_blocks);

    if (options.show_residual_summary) {
        residual_summary_.AddResidualInfo(RType_Prior, marginalization_factor, last_marginalization_parameter_blocks);
    }
}

void TrajectoryEstimator::AddIMUMeasurementAnalyticNURBS(
    const IMUData &imu_data, double *gyro_bias, double *accel_bias, double *gravity, const Eigen::Matrix<double, 6, 1> &info_vec, bool marg_this_factor)
{
    ...
    ceres::CostFunction *cost_function =
        new analytic_derivative::IMUFactorNURBS(time_ns, imu_data, gw, info_vec, trajectory_->knts, su, blending_matrix, cumulative_blending_matrix);
...
void TrajectoryEstimator::AddLoamMeasurementAnalyticNURBS(const PointCorrespondence &pc,
                                                          const SO3d &S_GtoM,
                                                          ...
                                                          bool marg_this_factor)
{
    ...
    ceres::CostFunction *cost_function = new Functor(time_ns, pc, su, blending_matrix, cumulative_blending_matrix, S_GtoM, p_GinM, S_LtoI, p_LinI, weight);
                                                         ...
                                                         double img_weight)
{
    ...
    ceres::CostFunction *cost_function =
        new Functor(time_ns, su, blending_matrix, cumulative_blending_matrix, visual_map_point, pixel_obs, S_VtoI, p_VinI, K, img_weight);
    ...
    double img_weight)
{
    ...
    ceres::CostFunction *cost_function = new Functor(patch_size_half, scale, level, time_ns, su, blending_matrix, cumulative_blending_matrix,
...
void TrajectoryEstimator::AddBiasFactor(double *bias_gyr_i,
                                        double *bias_gyr_j,
                                        double *bias_acc_i,
                                        double *bias_acc_j,
                                        double dt,
                                        ...
                                        bool marg_all_bias)
{
    analytic_derivative::BiasFactor *cost_function = new analytic_derivative::BiasFactor(dt, info_vec);
...
void TrajectoryEstimator::AddPoseMeasurementAnalyticDiffNURBS(const PoseData &pose_data, double pos_weight, double rot_weight)
{
    ...
    ceres::CostFunction *cost_function =
        new analytic_derivative::IMUPoseFactorNURBS(time_ns, pose_data, info_vec, trajectory_->knts, su, blending_matrix, cumulative_blending_matrix);
...
void TrajectoryEstimator::AddRelativePoseMeasurementAnalyticDiffNURBS(const PoseData &pose_data, double pos_weight, double rot_weight)
{
    int64_t time_ns = pose_data.timestamp;
    std::pair<int, double> su;  // i u
    trajectory_->GetIdxT(time_ns, su);
    ...
    ceres::CostFunction *cost_function = new analytic_derivative::IMURelativePoseFactorNURBS(time_ns, pose_data, pose_init, info_vec, trajectory_->knts, su,
...
void TrajectoryEstimator::AddCallback(const std::vector<std::string> &descriptions, const std::vector<size_t> &block_size, std::vector<double *> &param_block)
{
...
ceres::Solver::Summary TrajectoryEstimator::PreSolve(int max_iterations, bool progress, int num_threads)
{
    ...
    if (this->options.show_residual_summary) {
        residual_summary_.PrintSummary(trajectory_->minTimeNs(), trajectory_->getDtNs(), fixed_control_point_index_);
    }
...
/// not used
void ResidualSummary::AddResidualInfo(ResidualType r_type, const ceres::CostFunction *cost_function, const std::vector<double *> &param_vec)
{
...
void ResidualSummary::PrintSummary(int64_t t0_ns, int64_t dt_ns, int fixed_ctrl_idx) const
{
    ...
    for (auto typ = RType_Pose; typ <= RType_Prior; typ = ResidualType(typ + 1)) {
        ...
        if (num > 0 && t_span_ns.first > 0) {
            log.stream() << "\t- " << ResidualTypeStr[int(typ)] << ": " << GetCtrlString(t_span_ns.first, t_span_ns.second, t0_ns, dt_ns) << "\n";
        }
    }
    ...
    for (int k = 0; k < 2; k++) {
        ...
        log.stream() << GetTimeString(knot_span.first, knot_span.second, t0_ns, dt_ns) << "\n";
    }
    if (fixed_ctrl_idx > 0) {
        int64_t time_ns = (fixed_ctrl_idx - SplineOrder + 1) * dt_ns + t0_ns + dt_ns;

        log.stream() << "\t- fixed ctrl idx: " << fixed_ctrl_idx << "; opt time [" << time_ns * NS_TO_S << ", ~]\n";
    }
}

std::string ResidualSummary::GetTimeString(int64_t knot_min, int64_t knot_max, int64_t t0_ns, int64_t dt_ns) const
{
    int64_t t_min_ns = (knot_min - SplineOrder + 1) * dt_ns + t0_ns;
    int64_t t_max_ns = (knot_max - SplineOrder + 1) * dt_ns + t0_ns + (dt_ns);

    std::stringstream ss;
    ss << "[" << t_min_ns * NS_TO_S << ", " << t_max_ns * NS_TO_S << "] in [" << knot_min << ", " << knot_max << "]";
    std::string segment_info = ss.str();
    return segment_info;
}

std::string ResidualSummary::GetCtrlString(int64_t t_min_ns, int64_t t_max_ns, int64_t t0_ns, int64_t dt_ns) const
{
...
void TrajectoryEstimator::AddControlPoints(const SplineMeta<SplineOrder> &spline_meta, std::vector<double *> &vec, bool addPosKnot)
{
        ...
        for (size_t i = start_idx; i < (start_idx + seg.NumParameters()); ++i) {
            if (addPosKnot) {
                ...
                if (options.use_auto_diff) {
                    problem_->AddParameterBlock(vec.back(), 4, auto_diff_local_parameterization_);
                } else {
                    problem_->AddParameterBlock(vec.back(), 4, analytic_local_parameterization_);
                }
            }
...
void TrajectoryEstimator::PrepareMarginalizationInfo(ResidualType r_type,
                                                     ceres::CostFunction *cost_function,
                                                     ceres::LossFunction *loss_function,
                                                     std::vector<double *> &parameter_blocks,
                                                     std::vector<int> &drop_set)
{
    ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(r_type, cost_function, NULL, parameter_blocks, drop_set);
                                                     ...
                                                     std::vector<int> &drop_set_wo_ctrl_point)
{
    ...
    if (drop_set.size() > 0) {
        ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(r_type, cost_function, loss_function, parameter_blocks, drop_set);
        marginalization_info_->addResidualBlockInfo(residual_block_info);
    }
}
...
void TrajectoryEstimator::SaveMarginalizationInfo(MarginalizationInfo::Ptr &marg_info_out, std::vector<double *> &marg_param_blocks_out)
{
...
void TrajectoryEstimator::SetKeyScanConstant(double max_time)
{
    ...
    LOG(INFO) << "fixed_control_point_index: " << index << "/" << trajectory_->numKnots() << "; max_time: " << max_time << std::endl;
}

bool TrajectoryEstimator::MeasuredTimeToNs(const SensorType &sensor_type, const double &timestamp, int64_t &time_ns) const
{
    ...
    if (!options.lock_EPs.at(sensor_type).lock_t_offset) {
        ...
        //    t_min                          t_max
        if (time_ns - options.t_offset_padding_ns < t_min_traj_ns || time_ns + options.t_offset_padding_ns >= t_max_traj_ns) return false;
    } else {
...
{
    if (problem_->HasParameterBlock(values) && !problem_->IsParameterBlockConstant(const_cast<double *>(values))) {
        return true;
    } else {
        return false;
    }
...
void TrajectoryEstimator::SetTimeoffsetState()
{
    for (auto &sensor_t : t_offset_ns_opt_params_) {
        if (problem_->HasParameterBlock(sensor_t.second)) {
            if (options.lock_EPs.at(sensor_t.first).lock_t_offset) problem_->SetParameterBlockConstant(sensor_t.second);
        }
    }
}
...
// =========== IMU =========== //
void TrajectoryEstimator::AddPoseMeasurementAnalytic(const PoseData &pose_data, const Eigen::Matrix<double, 6, 1> &info_vec)
{
    int64_t time_ns;
    ...
    if (option_Ep.lock_t_offset) {
        trajectory_->CaculateSplineMeta({{t_corrected_ns, t_corrected_ns}}, spline_meta);
    } else {
        trajectory_->CaculateSplineMeta({{t_corrected_ns - options.t_offset_padding_ns, t_corrected_ns + options.t_offset_padding_ns}}, spline_meta);
    }

    ceres::CostFunction *cost_function = new analytic_derivative::IMUPoseFactor(time_ns, pose_data, spline_meta.segments.at(0), info_vec);
    ...
    const IMUData &imu_data, double *gyro_bias, double *accel_bias, double *gravity, const Eigen::Matrix<double, 6, 1> &info_vec, bool marg_this_factor)
{
    ...
    ceres::CostFunction *cost_function = new analytic_derivative::IMUFactor(time_ns, imu_data, spline_meta.segments.at(0), info_vec);
    ...
    if (options.is_marg_state && marg_this_factor) {
        ...
        if (options.marg_t_offset_param) drop_set_wo_ctrl_point.emplace_back(Knot_size + 3);  // t_offset
        PrepareMarginalizationInfo(RType_IMU, spline_meta, cost_function, loss_function, vec, drop_set_wo_ctrl_point);
    } else {
        problem_->AddResidualBlock(cost_function, loss_function, vec);
    }
...
void TrajectoryEstimator::AddGravityFactor(double *gravity, const Eigen::Vector3d &info_vec, bool marg_this_factor)
{
    Eigen::Map<Eigen::Vector3d const> gravity_now(gravity);

    analytic_derivative::GravityFactor *cost_function = new analytic_derivative::GravityFactor(gravity_now, info_vec);
    ...
    if (options.is_marg_state && marg_this_factor) {
        ...
        PrepareMarginalizationInfo(RType_Gravity, cost_function, NULL, vec, drop_set);
    } else {
        problem_->AddResidualBlock(cost_function, NULL, vec);
    }
...
void TrajectoryEstimator::AddRelativeRotationAnalytic(double ta, double tb, const SO3d &S_BtoA, const Eigen::Vector3d &info_vec)
{
    ...
    SplineMeta<SplineOrder> spline_meta;
    trajectory_->CaculateSplineMeta({{t_min, t_min}, {t_max, t_max}}, spline_meta);

    using Functor                      = analytic_derivative::RelativeOrientationFactor;
    ceres::CostFunction *cost_function = new Functor(S_BtoA, time_a_ns, time_b_ns, spline_meta, info_vec);
...
void TrajectoryEstimator::AddLocalVelocityMeasurementAnalytic(const double timestamp, const Eigen::Vector3d &local_v, double weight)
{
    int64_t time_ns;
    ...
    ceres::CostFunction *cost_function = new Functor(time_ns, local_v, spline_meta.segments.at(0), weight);
...
void TrajectoryEstimator::AddLocal6DoFVelocityAnalytic(const double timestamp,
                                                       const Eigen::Matrix<double, 6, 1> &local_v,
                                                       const Eigen::Matrix<double, 6, 1> &info_vec)
{
    int64_t time_ns;
    if (!MeasuredTimeToNs(IMUSensor, timestamp, time_ns)) return;
    ...
    ceres::CostFunction *cost_function = new Functor(time_ns, local_v, spline_meta.segments.at(0), info_vec);
...
void TrajectoryEstimator::AddLoamMeasurementAnalytic(const PointCorrespondence &pc,
                                                     const SO3d &S_GtoM,
                                                     const Eigen::Vector3d &p_GinM,
                                                     const SO3d &S_LtoI,
                                                     const Eigen::Vector3d &p_LinI,
                                                     ...
                                                     bool marg_this_factor)
{
    ...
    ceres::CostFunction *cost_function = new Functor(time_ns, pc, spline_meta.segments.at(0), S_GtoM, p_GinM, S_LtoI, p_LinI, weight);
    ...
    if (options.is_marg_state && marg_this_factor) {
        ...
        if (dist < 0.05) {
            std::vector<int> drop_set_wo_ctrl_point;
            PrepareMarginalizationInfo(RType_LiDAR, spline_meta, cost_function, NULL, vec, drop_set_wo_ctrl_point);
        }
    } else {
        problem_->AddResidualBlock(cost_function, NULL, vec);
    }
...
void TrajectoryEstimator::AddRalativeLoamFeatureAnalytic(const PointCorrespondence &pc, double weight, bool marg_this_factor)
{
    ...
    trajectory_->CaculateSplineMeta({{time_map_ns, time_map_ns}, {time_point_ns, time_point_ns}}, spline_meta);

    using Functor                      = analytic_derivative::RalativeLoamFeatureFactor;
    ceres::CostFunction *cost_function = new Functor(time_point_ns, time_map_ns, pc, spline_meta, weight);
    ...
    if (options.is_marg_state && marg_this_factor) {
        std::vector<int> drop_set_wo_ctrl_point;
        PrepareMarginalizationInfo(RType_LiDAR_Relative, spline_meta, cost_function, NULL, vec, drop_set_wo_ctrl_point);
    } else {
        problem_->AddResidualBlock(cost_function, NULL, vec);
    }
...
void TrajectoryEstimator::AddLoamFeatureOptMapPoseAnalytic(
    const PointCorrespondence &pc, double *S_ImtoG, double *p_IminG, double weight, bool marg_this_factor)
{
    int64_t time_ns;
    if (!MeasuredTimeToNs(LiDARSensor, pc.t_point, time_ns)) return;
    SplineMeta<SplineOrder> spline_meta;
    ...
    ceres::CostFunction *cost_function = new Functor(time_ns, pc, spline_meta.segments.at(0), weight);
    ...
    if (options.is_marg_state && marg_this_factor) {
        ...
        PrepareMarginalizationInfo(RType_LiDAROpt, spline_meta, cost_function, NULL, vec, drop_set_wo_ctrl_point);
    } else {
        problem_->AddResidualBlock(cost_function, NULL, vec);
    }
...
void TrajectoryEstimator::AddImageFeatureAnalytic(
    const double ti, const Eigen::Vector3d &pi, const double tj, const Eigen::Vector3d &pj, double *inv_depth, bool fixed_depth, bool marg_this_fearure)
{
    ...
    if (option_Ep.lock_t_offset) {
        ...
        trajectory_->CaculateSplineMeta({{t_min - options.t_offset_padding_ns, t_min + options.t_offset_padding_ns},
                                         {t_max - options.t_offset_padding_ns, t_max + options.t_offset_padding_ns}},
                                        spline_meta);
    }

    using Functor                      = analytic_derivative::ImageFeatureFactor;
    ceres::CostFunction *cost_function = new Functor(time_i_ns, pi, time_j_ns, pj, spline_meta);
    ...
    if (options.is_marg_state && marg_this_fearure) {
        ...
        if (options.marg_t_offset_param) drop_set_wo_ctrl_point.emplace_back(Knot_size + 1);  // t_offset
        PrepareMarginalizationInfo(RType_Image, spline_meta, cost_function, loss_function, vec, drop_set_wo_ctrl_point);
    } else {
        problem_->AddResidualBlock(cost_function, loss_function, vec);
    }
...
void TrajectoryEstimator::AddImage3D2DAnalytic(const double ti, const Eigen::Vector3d &pi, double *p_G, bool fixed_p_G, bool marg_this_fearure)
{
    int64_t time_i_ns;
    if (!MeasuredTimeToNs(CameraSensor, ti, time_i_ns)) return;
    double *t_offset_ns     = t_offset_ns_opt_params_[CameraSensor];
    ...
    SplineMeta<SplineOrder> spline_meta;
    if (option_Ep.lock_t_offset) {
        trajectory_->CaculateSplineMeta({{ti_corrected_ns, ti_corrected_ns}}, spline_meta);
    } else {
        trajectory_->CaculateSplineMeta({{ti_corrected_ns - options.t_offset_padding_ns, ti_corrected_ns + options.t_offset_padding_ns}}, spline_meta);
    }
...
void TrajectoryEstimator::AddImageFeatureOnePoseAnalytic(const Eigen::Vector3d &p_i,
                                                         const SO3d &S_IitoG,
                                                         ...
                                                         bool marg_this_fearure)
{
    ...
    ceres::CostFunction *cost_function = new Functor(p_i, S_IitoG, p_IiinG, time_j_ns, p_j, spline_meta);
...
void TrajectoryEstimator::AddImageDepthAnalytic(
    const Eigen::Vector3d &p_i, const Eigen::Vector3d &p_j, const SO3d &S_CitoCj, const Eigen::Vector3d &p_CiinCj, double *inv_depth)
{
    using Functor                      = analytic_derivative::ImageDepthFactor;
    ceres::CostFunction *cost_function = new Functor(p_i, p_j, S_CitoCj, p_CiinCj);
...
void TrajectoryEstimator::AddEpipolarFactorAnalytic(const double t_i,
                                                    ...
                                                    bool marg_this_fearure)
{
    ...
    ceres::CostFunction *cost_function = new Functor(t_i, x_i, x_k, S_GtoCk, p_CkinG, spline_meta, weight);
    ...
    if (options.is_marg_state && marg_this_fearure) {
        std::vector<int> drop_set_wo_ctrl_point;
        PrepareMarginalizationInfo(RType_Epipolar, spline_meta, cost_function, loss_function, vec, drop_set_wo_ctrl_point);
    } else {
        problem_->AddResidualBlock(cost_function, loss_function, vec);
    }
