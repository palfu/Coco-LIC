    ...
    , process_cur_img_(false)
{
    ...
    InitFactorInfo(trajectory_->GetSensorEP(CameraSensor), trajectory_->GetSensorEP(LiDARSensor), opt_weight_.image_weight,
                   opt_weight_.local_velocity_info_vec);
...
void TrajectoryManager::InitFactorInfo(const ExtrinsicParam &Ep_CtoI,
                                       ...
                                       const Eigen::Vector3d &local_velocity_weight)
{
    if (image_feature_weight > 1e-5) {
        ...
        analytic_derivative::ImageFeatureOnePoseFactor::SetParam(Ep_CtoI.so3, Ep_CtoI.p);
    ...
    analytic_derivative::LoamFeatureOptMapPoseFactor::SetParam(Ep_LtoI.so3, Ep_LtoI.p);
    analytic_derivative::RalativeLoamFeatureFactor::SetParam(Ep_LtoI.so3, Ep_LtoI.p);
}
...
void TrajectoryManager::SetOriginalPose(Eigen::Quaterniond q, Eigen::Vector3d p)
{
...
void TrajectoryManager::UpdateIMUInlio()
{
    ...
    for (auto rter = imu_data_.rbegin(); rter != imu_data_.rend(); ++rter) {
        if (rter->timestamp < t_max) {
            tparam_.lio_imu_idx[1]  = std::distance(imu_data_.begin(), rter.base()) - 1;
...
void TrajectoryManager::PredictTrajectory(int64_t scan_time_min, int64_t scan_time_max, int64_t traj_max_time_ns, int knot_add_num, bool non_uniform)
{
...
void TrajectoryManager::InitTrajWithPropagation()
{
    ...
    TrajectoryEstimator::Ptr estimator(new TrajectoryEstimator(trajectory_, option, "Init Traj"));

    estimator->SetFixedIndex(3);

    // [0] prior factor
    if (true && lidar_marg_info) {
        estimator->AddMarginalizationFactor(lidar_marg_info, lidar_marg_parameter_blocks);
    }
    ...
    LOG(INFO) << init_cnt << " TrajInit Successful/Unsuccessful steps: " << summary.num_successful_steps << "/" << summary.num_unsuccessful_steps;
}

bool TrajectoryManager::UpdateTrajectoryWithLIC(int lidar_iter,
                                                ...
                                                const int iteration)
{
    if (point_corrs.empty() || imu_data_.empty() || imu_data_.size() == 1) {
        LOG(WARNING) << " input empty data " << point_corrs.size() << ", " << imu_data_.size();
        return false;
    }
    ...
    option.lock_g                = true;
    option.show_residual_summary = verbose;
    TrajectoryEstimator::Ptr estimator(new TrajectoryEstimator(trajectory_, option, "Before LIO"));
    ...
    for (const auto &v : point_corrs) {
        ...
        if (use_lidar_scale) {
            estimator->AddLoamMeasurementAnalyticNURBS(v, S_GtoM, p_GinM, S_LtoI, p_LinI, opt_weight_.lidar_weight * v.scale);
        } else {
            estimator->AddLoamMeasurementAnalyticNURBS(v, S_GtoM, p_GinM, S_LtoI, p_LinI, opt_weight_.lidar_weight);
        }
    }
    ...
    for (int i = tparam_.lio_imu_idx[0]; i < tparam_.lio_imu_idx[1]; ++i) {
        if (imu_data_.at(i).timestamp < opt_min_t_ns) continue;
        if (imu_data_.at(i).timestamp >= opt_max_t_ns) continue;
        estimator->AddIMUMeasurementAnalyticNURBS(imu_data_.at(i), para_bg_vec[0], para_ba_vec[0], gravity_.data(), opt_weight_.imu_info_vec);
    }
    ...
    estimator->AddBiasFactor(para_bg_vec[0], para_bg_vec[1], para_ba_vec[0], para_ba_vec[1], 1, sqrt_info_);
    ...
    if (pnp_3ds.size() != 0) {
        ...
        for (int i = 0; i < pnp_3ds.size(); i++) {
            estimator->AddPnPMeasurementAnalyticNURBS(pnp_3ds[i], pnp_2ds[i], img_time_stamp, trajectory_->GetSensorEP(CameraSensor).so3,
                                                      trajectory_->GetSensorEP(CameraSensor).p, K_, opt_weight_.image_weight);
        }
    } else {
        process_cur_img_ = false;
    }
    ...
    LOG(INFO) << ++loam_cnt << " UpdateLio Successful/Unsuccessful steps: " << summary.num_successful_steps << "/" << summary.num_unsuccessful_steps;

    opt_cnt++;
    t_opt_sum += opt_time;

    LOG(INFO) << "[gyro_bias_new] " << all_imu_bias_.rbegin()->second.gyro_bias.x() << " " << all_imu_bias_.rbegin()->second.gyro_bias.y() << " "
              << all_imu_bias_.rbegin()->second.gyro_bias.z();
    LOG(INFO) << "[acce_bias_new] " << all_imu_bias_.rbegin()->second.accel_bias.x() << " " << all_imu_bias_.rbegin()->second.accel_bias.y() << " "
              << all_imu_bias_.rbegin()->second.accel_bias.z();

    return true;
}

void TrajectoryManager::UpdateLiDARAttribute(double scan_time_min, double scan_time_max)
{
...
void TrajectoryManager::UpdateLICPrior(const Eigen::aligned_vector<PointCorrespondence> &point_corrs)
{
    TrajectoryEstimatorOptions option;
    option.is_marg_state = true;

    TrajectoryEstimator::Ptr estimator(new TrajectoryEstimator(trajectory_, option));  // AddControlPoint
    ...
    if (lidar_marg_info) {
        ...
        if (!drop_set.empty()) {
            ...
            ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(RType_Prior, cost_function, NULL, lidar_marg_parameter_blocks, drop_set);
    ...
    for (int i = tparam_.lio_imu_idx[0]; i < tparam_.lio_imu_idx[1]; ++i) {
        ...
        if (!drop_set.empty()) {
            ceres::CostFunction *cost_function     = new analytic_derivative::IMUFactorNURBS(time_ns, imu_data_.at(i), gravity_, opt_weight_.imu_info_vec,
                                                                                             trajectory_->knts, su, blending_matrix, cumulative_blending_matrix);
            ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(RType_IMU, cost_function, NULL, vec, drop_set);
            marginalization_info->addResidualBlockInfo(residual_block_info);
        }
    }
    ...
    for (const auto &v : point_corrs) {
        ...
        if (!drop_set.empty()) {
            ...
            ceres::CostFunction *cost_function = new analytic_derivative::LoamFeatureFactorNURBS(time_ns, v, su, blending_matrix, cumulative_blending_matrix,
                                                                                                 S_GtoM, p_GinM, S_LtoI, p_LinI, weight);
            ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(RType_LiDAR, cost_function, NULL, vec, drop_set);
    ...
    ResidualBlockInfo *residual_block_info         = new ResidualBlockInfo(RType_Bias, cost_function, NULL, vec, drop_set);
    marginalization_info->addResidualBlockInfo(residual_block_info);
        ...
        if (!drop_set.empty()) {
            ...
            for (int i = 0; i < v_points_.size(); i++) {
                ceres::CostFunction *cost_function = new analytic_derivative::PnPFactorNURBS(
                    time_ns, su, blending_matrix, cumulative_blending_matrix, v_points_[i], px_obss_[i], trajectory_->GetSensorEP(CameraSensor).so3,
                    trajectory_->GetSensorEP(CameraSensor).p, K_, opt_weight_.image_weight);
                ceres::LossFunction *loss_function     = NULL;
                loss_function                          = new ceres::CauchyLoss(10.0);  // adopted from vins-mono
                ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(RType_Image, cost_function, loss_function, vec, drop_set);
                marginalization_info->addResidualBlockInfo(residual_block_info);
            }
        }
    }
