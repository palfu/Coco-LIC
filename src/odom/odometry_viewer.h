...
public:
    void SetPublisher(ros::NodeHandle &nh)
    {
        ...
        pub_target_cloud_        = nh.advertise<sensor_msgs::PointCloud2>("/lio/target_fea", 10);
        pub_source_cloud_        = nh.advertise<sensor_msgs::PointCloud2>("/lio/source_fea", 10);
        pub_map_corr_temp_cloud_ = nh.advertise<sensor_msgs::PointCloud2>("/liomap_corr_temp", 10);

        // dense feature cloud
        pub_target_dense_cloud_ = nh.advertise<sensor_msgs::PointCloud2>("/lio/target_dense_cloud", 10);
        pub_source_dense_cloud_ = nh.advertise<sensor_msgs::PointCloud2>("/lio/source_dense_cloud", 10);
        ...
        pub_spline_ctrl_cloud_        = nh.advertise<sensor_msgs::PointCloud2>("/spline/ctrl_cloud", 10);
        pub_spline_active_ctrl_cloud_ = nh.advertise<sensor_msgs::PointCloud2>("/spline/active_ctrl_cloud", 10);
        pub_spline_marg_ctrl_cloud_   = nh.advertise<sensor_msgs::PointCloud2>("/spline/marg_ctrl_cloud", 10);

        pub_spline_ctrl_type_[Init]       = nh.advertise<nav_msgs::Path>("/spline/init_ctrl_path", 10);
        pub_spline_ctrl_cloud_type_[Init] = nh.advertise<sensor_msgs::PointCloud2>("/spline/init_ctrl_cloud", 10);

        pub_spline_ctrl_type_[Loop]       = nh.advertise<nav_msgs::Path>("/spline/loop_ctrl_path", 10);
        pub_spline_ctrl_cloud_type_[Loop] = nh.advertise<sensor_msgs::PointCloud2>("/spline/loop_ctrl_cloud", 10);

        pub_latest_bias_vio_ = nh.advertise<sensor_msgs::Imu>("/imu/latest_vio_bias", 10);
        pub_latest_bias_lio_ = nh.advertise<sensor_msgs::Imu>("/imu/latest_lio_bias", 10);
        /// imu state estimator trajectory
        pub_imu_propogation_trajectory_ = nh.advertise<nav_msgs::Path>("/imu/propogation_trajectory", 10);

        /// ===== VIO ===== ///
        pub_img_landmarks_  = nh.advertise<sensor_msgs::PointCloud2>("/vio/landmarks", 10);
        pub_img_marg_cloud_ = nh.advertise<sensor_msgs::PointCloud2>("/vio/marg_cloud", 10);
        pub_vio_keyframe_   = nh.advertise<sensor_msgs::PointCloud2>("/vio/keyframe", 10);
    ...
    {
        if (pub_sub_visual_map_.getNumSubscribers() != 0 && sub_visual_map_cloud.size() > 0) {
    ...
    void PublishTF(Eigen::Quaterniond quat, Eigen::Vector3d pos, std::string from_frame, std::string to_frame)
    {
        ...
        tbr.sendTransform(tf::StampedTransform(transform, ros::Time::now(), to_frame, from_frame));
    }

    void PublishIMUData(Trajectory::Ptr trajectory, const Eigen::aligned_vector<IMUData> &imu_data, const IMUBias &bias, const Eigen::Vector3d &gravity)
    {
        ...
        for (auto const &v : imu_data) {
            ...
            Eigen::Vector3d w_b = trajectory->GetRotVelBody(v.timestamp) + bias.gyro_bias;
            Eigen::Vector3d a_w = trajectory->GetTransAccelWorld(v.timestamp);
            SE3d pose           = trajectory->GetIMUPose(v.timestamp);
            Eigen::Vector3d a_b = pose.so3().inverse() * (a_w + gravity) + bias.accel_bias;
    ...
    void PublishViconData(Trajectory::Ptr trajectory, Eigen::aligned_vector<PoseData> &vicon_data)
    {
    ...
    void PublishLoamMapCorrTemp(Trajectory::Ptr trajectory, const VPointCloud &map_corr_temp_cloud)
    {
    ...
    void PublishLoamCorrespondence(Trajectory::Ptr trajectory, const Eigen::aligned_vector<PointCorrespondence> &point_measurement)
    {
        if (pub_source_cloud_.getNumSubscribers() == 0 && pub_target_cloud_.getNumSubscribers() == 0) return;

        VPointCloud target_cloud, source_cloud;
        for (const PointCorrespondence &cor : point_measurement) {
            if (cor.t_map < 0.0 || cor.t_point < 0.0) {
                LOG(INFO) << "[PublishLoamCorrespondence] skip " << cor.t_map << "; " << cor.t_point;
                continue;
            }
    ...
    void PublishSplineType(Trajectory::Ptr trajectory, SplineViewerType viewer_type)
    {
        ...
        if (pub_spline_ctrl_type_[viewer_type].getNumSubscribers() != 0) {
            ...
            for (size_t i = start_idx; i < trajectory->numKnots(); ++i) {
                ...
                tf::quaternionEigenToMsg(trajectory->getKnotSO3(i).unit_quaternion(), geo_ctrl.pose.orientation);
    ...
    {
        if (pub_spline_marg_ctrl_cloud_.getNumSubscribers() != 0 && marg_ctrl_cloud.size() > 0) {
            sensor_msgs::PointCloud2 cloud_msg;
    ...
    void PublishSplineTrajectory(Trajectory::Ptr trajectory, double min_time, double max_time, double dt)
    {
        ros::Time time_now = ros::Time::now();
        ros::Time t_temp;
        ...
        if (pub_spline_trajectory_.getNumSubscribers() != 0) {
            ...
            for (double t = min_time; t < max_time - 0.08; t += dt) {
                ...
                tf::quaternionEigenToMsg(pose.unit_quaternion(), poseIinG.pose.orientation);
    ...
    void PublishLatestBias(const Eigen::Vector3d &gyro_bias, const Eigen::Vector3d &accel_bias, bool is_LIO = true)
    {
        if (pub_latest_bias_vio_.getNumSubscribers() != 0 || pub_latest_bias_lio_.getNumSubscribers() != 0) {
    ...
    void PublishOdomGt(const std::vector<PoseData> &poses_gt, double max_time)
    {
        ...
        for (auto const &v : poses_gt) {
            ...
            tf::quaternionEigenToMsg(v.orientation.unit_quaternion(), pose_gt.pose.orientation);
    ...
    void PublishIMUEstimator(const ImuStateEstimator::Ptr &imu_estimator)
    {
        ...
        poses_propogation.push_back(fromIMUState(imu_estimator->GetPropagateStartState()));
    ...
    {  // feature_cur_ds_
        static int skip = 0;
        if (pub_target_dense_cloud_.getNumSubscribers() != 0 && (++skip % 20 == 0)) {
            PosCloud target_cloud;
            Eigen::Matrix4d tranform_matrix = Eigen::Matrix4d::Identity();
            pcl::transformPointCloud(*target_feature.surface_features, target_cloud, tranform_matrix);
        ...
        if (pub_source_dense_cloud_.getNumSubscribers() != 0) {
            ...
            for (size_t i = 0; i < source_feature.surface_features->size(); i++) {
                int64_t point_timestamp = source_feature.surface_features->points[i].timestamp;
                ...
                Eigen::Vector3d point_local(source_feature.surface_features->points[i].x, source_feature.surface_features->points[i].y,
                                            source_feature.surface_features->points[i].z);
                Eigen::Vector3d point_out = point_pos.so3() * point_local + point_pos.translation();
                VPoint p;
                p.x         = point_out(0);
                p.y         = point_out(1);
                p.z         = point_out(2);
    ...
    void ErrorStatistics(Trajectory::Ptr trajectory,
                         ...
                         std::string note = " ")
    {
        ...
        int surface_cnt      = 0;
        for (const PointCorrespondence &cor : point_measurement) {
            if (cor.t_map < trajectory->minTime(LiDARSensor) || cor.t_point < trajectory->minTime(LiDARSensor)) {
                LOG(INFO) << "[PublishLoamCorrespondence] skip " << cor.t_map << "; " << cor.t_point;
                continue;
            }
