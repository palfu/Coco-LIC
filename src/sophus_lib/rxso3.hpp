...
public:
    ...
    //
    SOPHUS_FUNC Line operator*(Line const& l) const { return Line((*this) * l.origin(), (*this) * l.direction() / quaternion().squaredNorm()); }
    ...
    {
        SOPHUS_ENSURE(quat.squaredNorm() > Constants<Scalar>::epsilon() * Constants<Scalar>::epsilon(), "Scale factor must be greater-equal epsilon.");
    ...
    SOPHUS_FUNC void setScaledRotationMatrix(Transformation const& sR)
    {
        ...
        SOPHUS_ENSURE(squared_scale > Constants<Scalar>::epsilon() * Constants<Scalar>::epsilon(), "Scale factor must be greater-equal epsilon.");
