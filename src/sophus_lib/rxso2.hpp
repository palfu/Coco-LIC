...
public:
    ...
    SOPHUS_FUNC RxSO2Base<Derived>& operator*=(RxSO2<Scalar> const& other)
    {
        ...
        if (squared_scale < Constants<Scalar>::epsilon() * Constants<Scalar>::epsilon()) {
    ...
    {
        SOPHUS_ENSURE(z.squaredNorm() > Constants<Scalar>::epsilon() * Constants<Scalar>::epsilon(), "Scale factor must be greater-equal epsilon.");
...
public:
    ...
    //
    SOPHUS_FUNC RxSO2(Scalar const& scale, Transformation const& R) : RxSO2((scale * SO2<Scalar>(R).unit_complex()).eval()) {}
