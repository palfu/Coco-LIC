...
SOPHUS_FUNC bool isScaledOrthogonalAndPositive(Eigen::MatrixBase<D> const& sR)
{
    ...
    return (sR * sR.transpose() - scale_sqr * Matrix<Scalar, N, N>::Identity()).template lpNorm<Eigen::Infinity>() < sqrt(Constants<Scalar>::epsilon());
}

// Takes in arbiray square matrix (2x2 or larger) and returns closest
// orthogonal matrix with positive determinant.
template <class D>
SOPHUS_FUNC enable_if_t<std::is_floating_point<typename D::Scalar>::value, Matrix<typename D::Scalar, D::RowsAtCompileTime, D::RowsAtCompileTime>>
