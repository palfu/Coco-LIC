...
#define SOPHUS_ENSURE(expr, ...) \
    ((expr) ? ((void) 0) : ::Sophus::ensureFailed(SOPHUS_FUNCTION, __FILE__, __LINE__, Sophus::details::FormatString(##__VA_ARGS__).c_str()))
#else
// LCOV_EXCL_START
...
// LCOV_EXCL_STOP
#define SOPHUS_ENSURE(expr, ...) ((expr) ? ((void) 0) : Sophus::defaultEnsure(SOPHUS_FUNCTION, __FILE__, __LINE__, ##__VA_ARGS__))
#endif
...
struct IsUniformRandomBitGenerator {
    static const bool value =
        std::is_unsigned<typename G::result_type>::value && std::is_unsigned<decltype(G::min())>::value && std::is_unsigned<decltype(G::max())>::value;
};
}  // namespace Sophus
