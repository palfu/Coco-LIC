...
public:
    ...
    //
    SOPHUS_FUNC Point operator*(Point const& p) const { return rxso2() * p + translation(); }
...
public:
    ...
    SOPHUS_FUNC Sim2(RxSO2Base<OtherDerived> const& rxso2, Eigen::MatrixBase<D> const& translation) : rxso2_(rxso2), translation_(translation)
    {
        static_assert(std::is_same<typename OtherDerived::Scalar, Scalar>::value, "must be same Scalar type");
    ...
    //
    template <class D>
    SOPHUS_FUNC Sim2(Vector2<Scalar> const& complex_number, Eigen::MatrixBase<D> const& translation) : rxso2_(complex_number), translation_(translation)
    {
        static_assert(std::is_same<typename D::Scalar, Scalar>::value, "must be same Scalar type");
    }
    ...
    //
    SOPHUS_FUNC explicit Sim2(Matrix<Scalar, 3, 3> const& T) : rxso2_((T.template topLeftCorner<2, 2>()).eval()), translation_(T.template block<2, 1>(0, 2)) {}
...
Sim2<Scalar, Options>::Sim2() : translation_(TranslationMember::Zero())
{
    ...
    static_assert(offsetof(Sim2, rxso2_) + sizeof(Scalar) * RxSO2<Scalar>::num_parameters == offsetof(Sim2, translation_),
