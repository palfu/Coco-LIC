...
public:
    ...
    SOPHUS_FUNC TangentAndTheta logAndTheta() const
    {
        ...
        if (n < Constants<Scalar>::epsilon()) {
            ...
            SOPHUS_ENSURE(abs(w) >= Constants<Scalar>::epsilon(), "Quaternion (%) should be normalized!", unit_quaternion().coeffs().transpose());
    ...
    //
    SOPHUS_FUNC Point operator*(Point const& p) const { return unit_quaternion()._transformVector(p); }
...
public:
    ...
    //
    SOPHUS_FUNC static Sophus::Matrix<Scalar, DoF, num_parameters> Dx_exp_x(Tangent const& omega)
    {
        using std::cos;
        using std::exp;
        using std::sin;
    ...
    SOPHUS_FUNC static SO3<Scalar> expAndTheta(Tangent const& omega, Scalar* theta)
    {
        ...
        *theta            = sqrt(theta_sq);
        ...
        SO3 q;
        q.unit_quaternion_nonconst() = QuaternionMember(real_factor, imag_factor * omega.x(), imag_factor * omega.y(), imag_factor * omega.z());
        SOPHUS_ENSURE(abs(q.unit_quaternion().squaredNorm() - Scalar(1)) < Sophus::Constants<Scalar>::epsilon(), "SO3::exp failed! omega: %, real: %, img: %",
    ...
    {
        static_assert(IsUniformRandomBitGenerator<UniformRandomBitGenerator>::value, "generator must meet the UniformRandomBitGenerator concept");
