...
public:
   ...
   */
    ...
    //
    SOPHUS_FUNC Point operator*(Point const& p) const { return so2() * p + translation(); }
    ...
    SOPHUS_FUNC void setRotationMatrix(Matrix<Scalar, 2, 2> const& R)
    {
        ...
        so2().setComplex(Scalar(0.5) * (R(0, 0) + R(1, 1)), Scalar(0.5) * (R(1, 0) - R(0, 1)));
    }
...
public:
    ...
    SOPHUS_FUNC SE2(SO2Base<OtherDerived> const& so2, Eigen::MatrixBase<D> const& translation) : so2_(so2), translation_(translation)
    {
        static_assert(std::is_same<typename OtherDerived::Scalar, Scalar>::value, "must be same Scalar type");
    ...
    //
    SOPHUS_FUNC
    SE2(typename SO2<Scalar>::Transformation const& rotation_matrix, Point const& translation) : so2_(rotation_matrix), translation_(translation) {}
    ...
    //
    SOPHUS_FUNC explicit SE2(Transformation const& T) : so2_(T.template topLeftCorner<2, 2>().eval()), translation_(T.template block<2, 1>(0, 2)) {}
    ...
    //
    SOPHUS_FUNC static Sophus::Matrix<Scalar, DoF, num_parameters> Dx_exp_x(Tangent const& upsilon_theta)
    {
        using std::abs;
        using std::cos;
        using std::pow;
      ...
      J << o, o, i, o,
        ...
        J(2, 0)        = -c0;
        J(2, 1)        = c3;
        J(2, 2)        = -c2 * upsilon[1] + c6 * upsilon[0] - c8 * upsilon[0] + c9 * upsilon[1];
        J(2, 3)        = c2 * upsilon[0] + c6 * upsilon[1] - c8 * upsilon[1] - c9 * upsilon[0];
        return J;
    }
    ...
    SOPHUS_FUNC static SE2<Scalar> exp(Tangent const& a)
    {
        ...
        Vector2<Scalar> trans(sin_theta_by_theta * a[0] - one_minus_cos_theta_by_theta * a[1], one_minus_cos_theta_by_theta * a[0] + sin_theta_by_theta * a[1]);
