...
public:
    ...
    void FeatureCloudHandler(int64_t scan_timestamp,
    ...
    int64_t GetLatestFeatureTime() { return latest_feature_time_; }

    PosCloud::Ptr GetLatestSurfaceFeature()
    {
        if (local_feature_container_.find(latest_feature_time_) == local_feature_container_.end()) return nullptr;
    ...
    //
    const Eigen::aligned_vector<PointCorrespondence> &GetPointCorrespondence() const { return point_correspondence_ds_; }
    ...
    void FindNearbyKeyFrames(const int key, const int search_num, PosCloud::Ptr &cloud_in_G) const;
...
private:
    ...
    //
    void GetNearTimeKeyScanID(PosCloud::Ptr key_pos_selected, const int64_t cur_time) const;

    void TransformLiDARFeature(const LiDARFeature &lf_in, const Eigen::Matrix4d &T_in_to_out, LiDARFeature &lf_out) const;

    void DownsampleLiDARFeature(const LiDARFeature &lf_in, LiDARFeature &lf_out) const;
    ...
    bool FindCorrespondence(const LiDARFeature &lf_cur, const LiDARFeature &lf_cur_in_M);
...
private:
    ...
    pcl::KdTreeFLANN<PointT>::Ptr kdtree_surface_map_;
