...
struct by_timestamp {
    bool operator()(const PointCorrespondence &left, const PointCorrespondence &right) { return left.t_point < right.t_point; }
};
    ...
    , key_frame_updated_(false)
{
    ...
    cloud_reserved_time_ = yaml::GetValue<double>(map_node, "cloud_reserved_time", -1) * S_TO_NS;
...
void LidarHandler::GetLoamFeatureAssociation()
{
    ...
    if (use_corner_feature_) {
        trajectory_->UndistortScanInG(*feature_cur_ds_.corner_features, feature_cur_ds_.timestamp, *cur_lf_in_map.corner_features);
    }
    trajectory_->UndistortScanInG(*feature_cur_ds_.surface_features, feature_cur_ds_.timestamp, *cur_lf_in_map.surface_features);

    FindCorrespondence(feature_cur_ds_, cur_lf_in_map);

    std::sort(point_correspondence_.begin(), point_correspondence_.end(), by_timestamp());

    DownSampleCorrespondence();

    LOG(INFO) << "point correspondence : " << point_correspondence_.size() << "; point correspondence_ds : " << point_correspondence_ds_.size();
}
...
bool LidarHandler::UpdateKeyFrames()
{
    ...
    //
    for (auto iter = cache_feature_container_.begin(); iter != cache_feature_container_.end();) {
        /// [Case 1]
        ...
        if (local_feature_container_.empty()) {
            ...
            *(key_scan.full_cloud)       = *(iter->second.full_cloud);
            ...
            //
            trajectory_->UndistortScan(*(iter->second.corner_features), key_scan.timestamp, *(key_scan.corner_features));
            trajectory_->UndistortScan(*(iter->second.surface_features), key_scan.timestamp, *(key_scan.surface_features));
            trajectory_->UndistortScan(*(iter->second.full_cloud), key_scan.timestamp, *(key_scan.full_cloud));
        }
#if false
    if (local_feature_container_.size() < 10)
      ...
      local_feature_container_[key_scan.timestamp] = key_scan_ds;
        ...
        Eigen::Vector3d p  = trajectory_->GetLidarPoseNURBS(key_scan.timestamp).translation();
        key_pose.x         = p[0];
        key_pose.y         = p[1];
        key_pose.z         = p[2];
...
const bool LidarHandler::IsKeyFrame(int64_t time) const
{
    ...
    //
    if ((time - cloud_key_pos_->back().timestamp) > keyframe_time_second_) return true;
    ...
    if (angle_degree > keyframe_angle_degree_ || dist_meter > keyframe_dist_meter_) {
...
void LidarHandler::GetNearTimeKeyScanID(PosCloud::Ptr key_pos_selected, const int64_t cur_time) const
{
    ...
    for (int i = cloud_key_pos_->size() - 1; i >= 0; i--) {
        ...
        ///
        auto iter = std::find(t_added.begin(), t_added.end(), cloud_key_pos_->points[i].timestamp);
        if (t_added.end() == iter) {
...
void LidarHandler::GetNearDistKeyScanID(PosCloud::Ptr key_pos_selected)
{
    ...
    pcl::KdTreeFLANN<PosPoint>::Ptr kdtree_nearby_key_pos(new pcl::KdTreeFLANN<PosPoint>());
    kdtree_nearby_key_pos->setInputCloud(cloud_key_pos_);
    kdtree_nearby_key_pos->radiusSearch(cloud_key_pos_->back(), (double) keyframe_search_radius_, indices, sqr_dists);
...
void LidarHandler::TransformLiDARFeature(const LiDARFeature &lf_in, const Eigen::Matrix4d &T_in_to_out, LiDARFeature &lf_out) const
{
    if (use_corner_feature_) {
        pcl::transformPointCloud(*lf_in.corner_features, *lf_out.corner_features, T_in_to_out);
    }
    pcl::transformPointCloud(*lf_in.surface_features, *lf_out.surface_features, T_in_to_out);

    if (update_full_cloud_) {
        pcl::transformPointCloud(*lf_in.full_cloud, *lf_out.full_cloud, T_in_to_out);
    }
...
void LidarHandler::DownsampleLiDARFeature(const LiDARFeature &lf_in, LiDARFeature &lf_out) const
{
    lf_out.Clear();
    lf_out.timestamp = lf_in.timestamp;
    lf_out.time_max  = lf_in.time_max;
...
void LidarHandler::ExtractSurroundFeatures(const int64_t cur_time)
{
    ...
    if (local_feature_container_.size() == 1) {  //
        ...
        LiDARFeature lf_out;
        TransformLiDARFeature(local_feature_container_[kf_time], pose_cur_to_G.matrix(), lf_out);
        ...
        *feature_map_.surface_features += *(lf_out.surface_features);
        ...
        for (auto const &pos : key_pos_selected->points) {
            ...
            if (local_feature_container_.find(kf_time) != local_feature_container_.end()) {
                ...
                *feature_map_.surface_features += *lf_out.surface_features;
                ...
                feature_map_.time_max = std::max(feature_map_.time_max, lf_out.time_max);
            }
        }
    }
    ...
    LOG(INFO) << "[feature_map_ds] "
              << "surface cloud map : " << feature_map_ds_.surface_features->size() << "; cornor cloud map: " << feature_map_ds_.corner_features->size()
              << "; feature map time : [" << feature_map_ds_.timestamp << ", " << feature_map_ds_.time_max << "].";
...
{
    assert(!feature_map.surface_features->empty() && "[SetTargetMap] input surface_features are empty");
    assert(!(use_corner_feature_ && feature_map.corner_features->empty()) && "[SetTargetMap] input corner_features are empty");

    if (use_corner_feature_) kdtree_corner_map_->setInputCloud(feature_map.corner_features);
    kdtree_surface_map_->setInputCloud(feature_map.surface_features);
}

bool LidarHandler::FindCorrespondence(const LiDARFeature &lf_cur, const LiDARFeature &lf_cur_in_M)
{
    point_correspondence_.clear();
    point_correspondence_.reserve(lf_cur.surface_features->size());  //

    if ((use_corner_feature_ && lf_cur.corner_features->size() < size_t(edge_min_valid_num_)) &&
        lf_cur.surface_features->size() < size_t(surf_min_valid_num_)) {
        LOG(WARNING) << "[FindCorrespondence] No enough feature points ! Corner: " << lf_cur.corner_features->size()
    ...
    if (use_corner_feature_ && feature_map_ds_.corner_features->size() > 10) {
        for (size_t i = 0; i < lf_cur.corner_features->size(); i += corner_step) {
            ...
            if (matD1.at<float>(0, 0) > 3 * matD1.at<float>(0, 1)) {
                ...
                point_cor.point =
                    Eigen::Vector3d(lf_cur.corner_features->points[i].x, lf_cur.corner_features->points[i].y, lf_cur.corner_features->points[i].z);
                point_cor.scale = s;
    ...
    for (size_t i = 0; i < lf_cur.surface_features->size(); i += surf_step) {
        ...
        ///
        for (int j = 0; j < 5; j++) {
            if (fabs(pa * feature_map_ds_.surface_features->points[k_indices[j]].x + pb * feature_map_ds_.surface_features->points[k_indices[j]].y +
                     pc * feature_map_ds_.surface_features->points[k_indices[j]].z + pd) > 0.2) {
                planeValid = false;
                break;
            }
        }
        ...
        if (planeValid) {
            ...
            point_cor.point = Eigen::Vector3d(lf_cur.surface_features->points[i].x, lf_cur.surface_features->points[i].y, lf_cur.surface_features->points[i].z);
    ...
    LOG(INFO) << "point correspondence: " << point_correspondence_.size() << "; corner/surfel " << corner_feature_num << "/" << surf_feature_num;
    return true;
}
...
void LidarHandler::FindNearbyKeyFrames(const int key, const int search_num, PosCloud::Ptr &cloud_in_G) const
{
    cloud_in_G->clear();
    ...
    for (int i = -search_num; i <= search_num; i++) {
        ...
        if (local_feature_container_all_ds_.find(kf_time) == local_feature_container_all_ds_.end()) continue;
...
void LidarHandler::UpdateCloudKeyPos(const std::pair<int, int> &cur_wrt_history)
{
    // update key_pose
    int64_t t_cur     = cloud_key_pos_xy_->points[cur_wrt_history.first].timestamp;
    int64_t t_history = cloud_key_pos_xy_->points[cur_wrt_history.second].timestamp;
