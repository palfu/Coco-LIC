...
VelodyneFeatureExtraction::VelodyneFeatureExtraction(const YAML::Node &node) : fea_param_(LiDARFeatureParam(node["VLP16"]))
{
    AllocateMemory();
    ResetParameters();
}
...
void VelodyneFeatureExtraction::LidarHandler(const RTPointCloud::Ptr raw_cloud)
{
    p_corner_cloud.reset(new RTPointCloud());
    p_surface_cloud.reset(new RTPointCloud());
...
bool VelodyneFeatureExtraction::ParsePointCloudNoFeature(PointCloudMsg::Ptr &lidar_msg, RTPointCloud::Ptr out_cloud)
{
    int plsize = lidar_msg->cloud_->points.size();
    std::cout << "[plsize] " << plsize << std::endl;

    auto tmp_out_cloud = lidar_msg->cloud_;
    p_corner_cloud.reset(new RTPointCloud());
    p_surface_cloud.reset(new RTPointCloud());
    p_full_cloud.reset(new RTPointCloud());
    for (int i = 0; i < plsize; i++) {
        ...
        if (i % 4 == 0) {
            if (pt.x * pt.x + pt.y * pt.y + pt.z * pt.z > (2.0 * 2.0)) {
                p_surface_cloud->push_back(pt);
            }
        }
    }
...
void VelodyneFeatureExtraction::OrganizedCloudToRangeImage(const RTPointCloud::Ptr cur_cloud, cv::Mat &dist_image, RTPointCloud::Ptr &corresponding_cloud) const
{
    for (size_t column_id = 0; column_id < cur_cloud->width; ++column_id) {
        for (size_t row_id = 0; row_id < cur_cloud->height; ++row_id) {
            ...
            if (range < fea_param_.min_distance || range > fea_param_.max_distance) continue;
...
void VelodyneFeatureExtraction::RTCloudToRangeImage(const RTPointCloud::Ptr cur_cloud, cv::Mat &dist_image, RTPointCloud::Ptr &corresponding_cloud) const
{
    ...
    for (const RTPoint &p : cur_cloud->points) {
        ...
        int column_id = -round((horizon_angle - 90.0) / angle_resolution) + horizon_scan / 2;
    ...
    for (int i = 0; i < n_scan; i++) {
        ...
        for (int j = 0; j < horizon_scan; j++) {
            if (range_mat.at<float>(i, j) != FLT_MAX) {
                ...
                p_extracted_cloud->push_back(p_full_cloud->points[j + i * horizon_scan]);
...
void VelodyneFeatureExtraction::CaculateSmoothness()
{
    for (size_t i = 5; i < p_extracted_cloud->points.size() - 5; i++) {
        float diff_range = point_range_list[i - 5] + point_range_list[i - 4] + point_range_list[i - 3] + point_range_list[i - 2] + point_range_list[i - 1] -
                           point_range_list[i] * 10 + point_range_list[i + 1] + point_range_list[i + 2] + point_range_list[i + 3] + point_range_list[i + 4] +
...
void VelodyneFeatureExtraction::MarkOccludedPoints()
{
    for (size_t i = 5; i < p_extracted_cloud->points.size() - 6; i++) {
        ...
        int column_diff = std::abs(int(point_column_id[i + 1] - point_column_id[i]));
        ...
        float diff1 = std::abs(float(point_range_list[i - 1] - point_range_list[i]));
        float diff2 = std::abs(float(point_range_list[i + 1] - point_range_list[i]));

        if (diff1 > 0.02 * point_range_list[i] && diff2 > 0.02 * point_range_list[i]) cloud_neighbor_picked[i] = 1;
    }
}
    ...
    for (int i = 0; i < n_scan; i++) {
        ...
        for (int j = 0; j < 6; j++) {
            ...
            for (int k = ep; k >= sp; k--) {
                ...
                if (cloud_neighbor_picked[index] == 0 && cloud_curvature[index] > fea_param_.edge_threshold) {
                    largest_picked_num++;
                    ...
                    for (int l = 1; l <= 5; l++) {
                        int column_diff = std::abs(int(point_column_id[index + l] - point_column_id[index + l - 1]));
            ...
            for (int k = sp; k <= ep; k++) {
                int index = cloud_smoothness[k].ind;
                if (cloud_neighbor_picked[index] == 0 && cloud_curvature[index] < fea_param_.surf_threshold) {
                    cloud_label[index]           = -1;
                    cloud_neighbor_picked[index] = 1;
