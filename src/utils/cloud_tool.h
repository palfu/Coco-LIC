...
inline double GetCloudMaxTime(const RTPointCloud::Ptr cloud)
{
    assert(cloud->size() > 0 && "[GetCloudMaxTime] input empty cloud.");
    double t_max = cloud->back().timestamp;
    for (size_t i = cloud->size() - 1; i > 0; i--) {
        double t_point = cloud->points[i].timestamp;
        if (t_max < t_point) t_max = t_point;
    }
    return t_max;
}

inline void FilterCloudByTimestamp(const RTPointCloud::Ptr& cloud_in, double timestamp, RTPointCloud::Ptr& cloud_bef, RTPointCloud::Ptr& cloud_aft)
{
...
inline void CloudToRelativeMeasureTime(RTPointCloud::Ptr& cloud, double traj_start_time, double duration = 0.11)
{
    PointT zero_point;
    zero_point.x         = 0;
    zero_point.y         = 0;
    zero_point.intensity = 0;
    zero_point.timestamp = -1;
    for (size_t i = 0; i < cloud->size(); i++) {
        auto& dst             = cloud->points[i];
        auto t_wrt_traj_start = dst.timestamp - traj_start_time;
        if (t_wrt_traj_start) continue;
        if (t_wrt_traj_start > duration) {
            dst      = zero_point;
            dst.ring = dst.ring;
        } else {
            dst.timestamp = t_wrt_traj_start;
        }
    }
}
inline void RTPointCloudToPosCloud(const RTPointCloud::Ptr input_cloud, PosCloud::Ptr output_cloud, double* max_time = NULL)
{
    assert(input_cloud->size() > 0 && "[RTPointCloudToPosCloud] input empty cloud.");
...
public:
    ...
    void SetInputCloud(const typename pcl::PointCloud<PointType>::Ptr input_cloud) { input_cloud_ = input_cloud; }
    ...
    void Filter(typename pcl::PointCloud<PointType>::Ptr output_cloud)
    {
        ...
        for (unsigned int i = 0; i < input_cloud_->points.size(); i++) {
            ...
            int ijk0 = static_cast<int>(floor(p.x * inverse_resolution_) - static_cast<float>(min_b_[0]));
            int ijk1 = static_cast<int>(floor(p.y * inverse_resolution_) - static_cast<float>(min_b_[1]));
            int ijk2 = static_cast<int>(floor(p.z * inverse_resolution_) - static_cast<float>(min_b_[2]));
        ...
        for (auto leaf : first_and_last_indices_vector) {
            ...
            for (unsigned int i = leaf.first; i < leaf.second; i++) {
                centroid +=
                    Eigen::Vector3f(input_cloud_->points[index_vector[i].cloud_point_index].x, input_cloud_->points[index_vector[i].cloud_point_index].y,
